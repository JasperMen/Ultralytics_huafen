...
from timm.layers import LayerType, Linear, PadType, SelectAdaptivePool2d, create_conv2d, get_norm_act_layer
...
class MobileNetV3(nn.Module):
    ...
    'efficient head', where global pooling is done before the head convolution without a final batch-norm layer before
    the classifier.
    ...
    * MobileNet-V4 - https://arxiv.org/abs/2404.10518
    ...
    def group_matcher(self, coarse: bool = False) -> dict[str, Any]:
        ...
        return {"stem": r"^conv_stem|bn1", "blocks": r"^blocks\.(\d+)" if coarse else r"^blocks\.(\d+)\.(\d+)"}
...
class MobileNetV3Features(nn.Module):
    ...
    A work-in-progress feature extraction module for MobileNet-V3 to use as a backbone for segmentation and object
    detection models.
    """
...
def _gen_mobilenet_v3_rw(
    ...
    Ref impl: ? Paper: https://arxiv.org/abs/1905.02244
...
default_cfgs = generate_default_cfgs(
    {
        ...
        "mobilenetv4_conv_small.e2400_r224_in1k": _cfg(
            hf_hub_id="timm/", test_input_size=(3, 256, 256), test_crop_pct=0.95, interpolation="bicubic"
        ),
        ...
        "mobilenetv4_conv_medium.e500_r224_in1k": _cfg(
            hf_hub_id="timm/", crop_pct=0.95, test_input_size=(3, 256, 256), test_crop_pct=1.0, interpolation="bicubic"
        ),
        "mobilenetv4_conv_medium.e250_r384_in12k_ft_in1k": _cfg(
            hf_hub_id="timm/", input_size=(3, 384, 384), pool_size=(12, 12), crop_pct=0.95, interpolation="bicubic"
        ),
