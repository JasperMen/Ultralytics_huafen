...
class ConvBN(nn.Sequential):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.add_module(
            "conv", nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, **dd, **kwargs)
        )
        if with_bn:
...
class StarNet(nn.Module):
    ...
    def group_matcher(self, coarse: bool = False) -> dict[str, Any]:
        matcher = {
            "stem": r"^stem\.\d+",
            "blocks": [(r"^stages\.(\d+)" if coarse else r"^stages\.(\d+)\.(\d+)", None), (r"norm", (99999,))],
        }
        return matcher
...
def starnet_s1(pretrained: bool = False, **kwargs: Any) -> StarNet:
    model_args = {"base_dim": 24, "depths": [2, 2, 8, 3]}
...
def starnet_s2(pretrained: bool = False, **kwargs: Any) -> StarNet:
    model_args = {"base_dim": 32, "depths": [1, 2, 6, 2]}
...
def starnet_s3(pretrained: bool = False, **kwargs: Any) -> StarNet:
    model_args = {"base_dim": 32, "depths": [2, 2, 8, 4]}
...
def starnet_s4(pretrained: bool = False, **kwargs: Any) -> StarNet:
    model_args = {"base_dim": 32, "depths": [3, 3, 12, 5]}
...
def starnet_s050(pretrained: bool = False, **kwargs: Any) -> StarNet:
    model_args = {"base_dim": 16, "depths": [1, 1, 3, 1], "mlp_ratio": 3}
...
def starnet_s100(pretrained: bool = False, **kwargs: Any) -> StarNet:
    model_args = {"base_dim": 20, "depths": [1, 2, 4, 1], "mlp_ratio": 4}
...
def starnet_s150(pretrained: bool = False, **kwargs: Any) -> StarNet:
    model_args = {"base_dim": 24, "depths": [1, 2, 4, 2], "mlp_ratio": 3}
