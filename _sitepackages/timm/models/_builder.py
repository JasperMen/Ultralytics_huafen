...
from timm.models._features import FeatureDictNet, FeatureGetterNet, FeatureHookNet, FeatureListNet
...
def load_custom_pretrained(
    ...
    Downloads checkpoint file into cache-dir like torch.hub based loaders, but calls a passed in custom load fun, or the
    `load_pretrained` model member fn.

    If the object is already present in `model_dir`, it's deserialized and returned. The default value of `model_dir` is
    ``<hub_dir>/checkpoints`` where `hub_dir` is the directory returned by :func:`~torch.hub.get_dir`.
...
def build_model_with_cfg(
    model_cls: type[ModelT] | Callable[..., ModelT],
    variant: str,
    pretrained: bool,
