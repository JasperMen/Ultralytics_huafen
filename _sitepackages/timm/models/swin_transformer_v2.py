...
class WindowAttention(nn.Module):
    ...
    Supports both shifted and non-shifted window attention with continuous relative position bias and cosine attention.
    """
    ...
    def __init__(
        ...
        self.cpb_mlp = nn.Sequential(
            nn.Linear(2, 512, bias=True, **dd), nn.ReLU(inplace=True), nn.Linear(512, num_heads, bias=False, **dd)
        )
...
class SwinTransformerV2Block(nn.Module):
    ...
    A standard transformer block with window attention and shifted window attention for modeling long-range dependencies
    efficiently.
    """
    ...
    def _calc_window_shift(
        ...
        target_shift_size: _int_or_tuple_2_t | None = None,
...
class PatchMerging(nn.Module):
    ...
    Merges 2x2 neighboring patches and projects to higher dimension, effectively downsampling the feature maps.
    """
...
class SwinTransformerV2Stage(nn.Module):
    ...
    A single stage consisting of multiple Swin Transformer blocks with optional downsampling at the beginning.
    """
    ...
    def __init__(
        ...
        attn_drop: float = 0.0,
        drop_path: float = 0.0,
        act_layer: str | type[nn.Module] = "gelu",
        norm_layer: type[nn.Module] = nn.LayerNorm,
        pretrained_window_size: _int_or_tuple_2_t = 0,
...
class SwinTransformerV2(nn.Module):
    ...
    A hierarchical vision transformer using shifted windows for efficient self-attention computation with continuous
    position bias.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(self.num_layers):
            ...
            if i > 0:
                scale *= 2
            self.feature_info += [{"num_chs": out_dim, "reduction": 4 * scale, "module": f"layers.{i}"}]
...
def checkpoint_filter_fn(state_dict: dict[str, torch.Tensor], model: nn.Module) -> dict[str, torch.Tensor]:
    ...
    Handles resizing of patch embeddings and relative position tables when model size differs from checkpoint.
...
def swinv2_tiny_window16_256(pretrained: bool = False, **kwargs) -> SwinTransformerV2:
    ...
    model_args = {"window_size": 16, "embed_dim": 96, "depths": (2, 2, 6, 2), "num_heads": (3, 6, 12, 24)}
    return _create_swin_transformer_v2("swinv2_tiny_window16_256", pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def swinv2_tiny_window8_256(pretrained: bool = False, **kwargs) -> SwinTransformerV2:
    """Swin-T V2 @ 256x256, window 8x8."""
    model_args = {"window_size": 8, "embed_dim": 96, "depths": (2, 2, 6, 2), "num_heads": (3, 6, 12, 24)}
    return _create_swin_transformer_v2("swinv2_tiny_window8_256", pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def swinv2_small_window16_256(pretrained: bool = False, **kwargs) -> SwinTransformerV2:
    """Swin-S V2 @ 256x256, window 16x16."""
    model_args = {"window_size": 16, "embed_dim": 96, "depths": (2, 2, 18, 2), "num_heads": (3, 6, 12, 24)}
    return _create_swin_transformer_v2("swinv2_small_window16_256", pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def swinv2_small_window8_256(pretrained: bool = False, **kwargs) -> SwinTransformerV2:
    """Swin-S V2 @ 256x256, window 8x8."""
    model_args = {"window_size": 8, "embed_dim": 96, "depths": (2, 2, 18, 2), "num_heads": (3, 6, 12, 24)}
    return _create_swin_transformer_v2("swinv2_small_window8_256", pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def swinv2_base_window16_256(pretrained: bool = False, **kwargs) -> SwinTransformerV2:
    """Swin-B V2 @ 256x256, window 16x16."""
    model_args = {"window_size": 16, "embed_dim": 128, "depths": (2, 2, 18, 2), "num_heads": (4, 8, 16, 32)}
    return _create_swin_transformer_v2("swinv2_base_window16_256", pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def swinv2_base_window8_256(pretrained: bool = False, **kwargs) -> SwinTransformerV2:
    """Swin-B V2 @ 256x256, window 8x8."""
    model_args = {"window_size": 8, "embed_dim": 128, "depths": (2, 2, 18, 2), "num_heads": (4, 8, 16, 32)}
    return _create_swin_transformer_v2("swinv2_base_window8_256", pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def swinv2_base_window12_192(pretrained: bool = False, **kwargs) -> SwinTransformerV2:
    """Swin-B V2 @ 192x192, window 12x12."""
    model_args = {"window_size": 12, "embed_dim": 128, "depths": (2, 2, 18, 2), "num_heads": (4, 8, 16, 32)}
    return _create_swin_transformer_v2("swinv2_base_window12_192", pretrained=pretrained, **dict(model_args, **kwargs))
...
def swinv2_large_window12_192(pretrained: bool = False, **kwargs) -> SwinTransformerV2:
    ...
    model_args = {"window_size": 12, "embed_dim": 192, "depths": (2, 2, 18, 2), "num_heads": (6, 12, 24, 48)}
    return _create_swin_transformer_v2("swinv2_large_window12_192", pretrained=pretrained, **dict(model_args, **kwargs))
