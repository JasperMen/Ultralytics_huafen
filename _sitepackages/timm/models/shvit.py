...
from timm.layers import GroupNorm1, SelectAdaptivePool2d, SqueezeExcite, trunc_normal_
...
class Conv2dNorm(nn.Sequential):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.add_module(
            "c", nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, bias=False, **dd, **kwargs)
        )
...
class SHViT(nn.Module):
    def __init__(
        ...
        for i in range(len(embed_dim)):
            ...
            self.feature_info.append({"num_chs": prev_chs, "reduction": 2 ** (i + 4), "module": f"stages.{i}"})
