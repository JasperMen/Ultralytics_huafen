...
from timm.layers import DropPath, get_act_layer, ndgrid, to_2tuple, to_ntuple, trunc_normal_
...
class ConvNorm(nn.Module):
    ...
    def fuse(self):
        ...
        m = nn.Conv2d(
            ...
            w.size(0),
            w.shape[2:],
            stride=self.linear.stride,
            padding=self.linear.padding,
            dilation=self.linear.dilation,
            groups=self.linear.groups,
        )
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        resolution: int | tuple[int, int] = 14,
        use_conv: bool = False,
        ...
        self.proj = nn.Sequential(
            OrderedDict([("act", act_layer()), ("ln", ln_layer(self.val_attn_dim, dim, bn_weight_init=0, **dd))])
        )
    ...
    def _init_buffers(self) -> None:
        ...
        self.attention_bias_idxs.copy_(self._compute_attention_bias_idxs(device=self.attention_bias_idxs.device))
        self.attention_bias_cache = {}
    ...
    def forward(self, x):  # x (B,C,H,W)
        if self.use_conv:
            B, C, H, W = x.shape
            q, k, v = (
                self.qkv(x).view(B, self.num_heads, -1, H * W).split([self.key_dim, self.key_dim, self.val_dim], dim=2)
            )
            ...
            B, N, _C = x.shape
            q, k, v = (
                self.qkv(x).view(B, N, self.num_heads, -1).split([self.key_dim, self.key_dim, self.val_dim], dim=3)
            )
            q = q.permute(0, 2, 1, 3)
            k = k.permute(0, 2, 3, 1)
            v = v.permute(0, 2, 1, 3)
...
class AttentionDownsample(nn.Module):
    ...
    def __init__(
        ...
        if self.use_conv:
            ln_layer = ConvNorm
            sub_layer = partial(
                nn.AvgPool2d, kernel_size=3 if use_pool else 1, padding=1 if use_pool else 0, count_include_pad=False
            )
        else:
            ln_layer = LinearNorm
        ...
        self.q = nn.Sequential(
            OrderedDict([("down", sub_layer(stride=stride)), ("ln", ln_layer(in_dim, self.key_attn_dim, **dd))])
        )
        self.proj = nn.Sequential(
            OrderedDict([("act", act_layer()), ("ln", ln_layer(self.val_attn_dim, out_dim, **dd))])
        )
        ...
        self.register_buffer(
            "attention_bias_idxs", torch.empty((N_q, N_k), device=device, dtype=torch.long), persistent=False
        )
        self.attention_bias_cache = {}
...
class LevitDownsample(nn.Module):
    def __init__(
        ...
        mlp_ratio: float = 2.0,
        act_layer: type[nn.Module] = nn.SiLU,
        attn_act_layer: type[nn.Module] | None = None,
        resolution: int | tuple[int, int] = 14,
        use_conv: bool = False,
        use_pool: bool = False,
        drop_path: float = 0.0,
...
class Levit(nn.Module):
    ...
    NOTE: distillation is defaulted to True since pretrained weights use it, will cause problems w/ train scripts that
    don't take tuple outputs,
    """
    ...
    def __init__(
        ...
        stem_backbone: nn.Module | None = None,
        ...
        for i in range(num_stages):
            ...
            self.feature_info += [{"num_chs": embed_dim[i], "reduction": stride, "module": f"stages.{i}"}]
            in_dim = embed_dim[i]
    ...
    def reset_classifier(self, num_classes: int, global_pool: str | None = None):
        ...
        if global_pool is not None:
            self.global_pool = global_pool
        self.head = (
            NormLinear(self.num_features, num_classes, drop=self.drop_rate) if num_classes > 0 else nn.Identity()
        )
...
class LevitDistilled(Levit):
    def __init__(self, *args, **kwargs):
        ...
        self.head_dist = (
            NormLinear(self.num_features, self.num_classes, **dd) if self.num_classes > 0 else nn.Identity()
        )
...
model_cfgs = {
    "levit_128s": {"embed_dim": (128, 256, 384), "key_dim": 16, "num_heads": (4, 6, 8), "depth": (2, 3, 4)},
    "levit_128": {"embed_dim": (128, 256, 384), "key_dim": 16, "num_heads": (4, 8, 12), "depth": (4, 4, 4)},
    "levit_192": {"embed_dim": (192, 288, 384), "key_dim": 32, "num_heads": (3, 5, 6), "depth": (4, 4, 4)},
    "levit_256": {"embed_dim": (256, 384, 512), "key_dim": 32, "num_heads": (4, 6, 8), "depth": (4, 4, 4)},
    "levit_384": {"embed_dim": (384, 512, 768), "key_dim": 32, "num_heads": (6, 9, 12), "depth": (4, 4, 4)},
