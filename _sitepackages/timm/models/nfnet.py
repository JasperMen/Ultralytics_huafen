...
__all__ = ["NfCfg", "NormFreeNet"]  # model_registry will add each entrypoint fn to this
...
def create_stem(
    ...
    dd = {"device": device, "dtype": dtype}
    stem_stride = 2
    stem_feature = {"num_chs": out_chs, "reduction": 2, "module": "stem.conv"}
    stem = OrderedDict()
    ...
    if "deep" in stem_type:
        if "quad" in stem_type:
            ...
            strides = (2, 1, 1, 2)
            stem_stride = 4
            stem_feature = {"num_chs": out_chs // 2, "reduction": 2, "module": "stem.conv3"}
        else:
            ...
            strides = (2, 1, 1)
            stem_feature = {"num_chs": out_chs // 2, "reduction": 2, "module": "stem.conv2"}
...
class NormFreeNet(nn.Module):
    ...
    This model aims to cover both the NFRegNet-Bx models as detailed in the paper's code snippets and the (preact)
    ResNet models described earlier in the paper.
    ...
    def __init__(
        ...
        **kwargs: Any,
            ...
            for block_idx in range(cfg.depths[stage_idx]):
                ...
                blocks += [
                    NormFreeBlock(
                        ...
                        bottle_ratio=1.0 if cfg.reg and first_block else cfg.bottle_ratio,
                        ch_div=cfg.ch_div,
                        reg=cfg.reg,
                ...
                if block_idx == 0:
                    expected_var = 1.0  # expected var is reset after first block of each stage
                expected_var += cfg.alpha**2  # Even if reset occurs, increment expected variance
                first_dilation = dilation
                prev_chs = out_chs
            self.feature_info += [{"num_chs": prev_chs, "reduction": net_stride, "module": f"stages.{stage_idx}"}]
        ...
        if cfg.num_features:
            ...
            self.feature_info[-1] = {"num_chs": self.num_features, "reduction": net_stride, "module": "final_conv"}
        else:
...
def _nfnet_cfg(
    ...
    attn_kwargs = attn_kwargs if attn_kwargs is not None else {"rd_ratio": 0.5}
    cfg = NfCfg(
        depths=depths,
        channels=channels,
        stem_type="deep_quad",
...
model_cfgs = {
    ...
    "nfnet_l0": _nfnet_cfg(
        ...
        group_size=64,
        bottle_ratio=0.25,
        attn_kwargs={"rd_ratio": 0.25, "rd_divisor": 8},
        act_layer="silu",
    ),
    ...
    "nf_seresnet26": _nfres_cfg(depths=(2, 2, 2, 2), attn_layer="se", attn_kwargs={"rd_ratio": 1 / 16}),
    "nf_seresnet50": _nfres_cfg(depths=(3, 4, 6, 3), attn_layer="se", attn_kwargs={"rd_ratio": 1 / 16}),
    "nf_seresnet101": _nfres_cfg(depths=(3, 4, 23, 3), attn_layer="se", attn_kwargs={"rd_ratio": 1 / 16}),
    "nf_ecaresnet26": _nfres_cfg(depths=(2, 2, 2, 2), attn_layer="eca", attn_kwargs={}),
    "nf_ecaresnet50": _nfres_cfg(depths=(3, 4, 6, 3), attn_layer="eca", attn_kwargs={}),
    "nf_ecaresnet101": _nfres_cfg(depths=(3, 4, 23, 3), attn_layer="eca", attn_kwargs={}),
...
default_cfgs = generate_default_cfgs(
    {
        ...
        "nfnet_f0": _dcfg(url="", pool_size=(6, 6), input_size=(3, 192, 192), test_input_size=(3, 256, 256)),
        "nfnet_f1": _dcfg(url="", pool_size=(7, 7), input_size=(3, 224, 224), test_input_size=(3, 320, 320)),
        "nfnet_f2": _dcfg(url="", pool_size=(8, 8), input_size=(3, 256, 256), test_input_size=(3, 352, 352)),
        "nfnet_f3": _dcfg(url="", pool_size=(10, 10), input_size=(3, 320, 320), test_input_size=(3, 416, 416)),
        "nfnet_f4": _dcfg(url="", pool_size=(12, 12), input_size=(3, 384, 384), test_input_size=(3, 512, 512)),
        "nfnet_f5": _dcfg(url="", pool_size=(13, 13), input_size=(3, 416, 416), test_input_size=(3, 544, 544)),
        "nfnet_f6": _dcfg(url="", pool_size=(14, 14), input_size=(3, 448, 448), test_input_size=(3, 576, 576)),
        "nfnet_f7": _dcfg(url="", pool_size=(15, 15), input_size=(3, 480, 480), test_input_size=(3, 608, 608)),
        ...
        "eca_nfnet_l3": _dcfg(
            url="", pool_size=(11, 11), input_size=(3, 352, 352), test_input_size=(3, 448, 448), test_crop_pct=1.0
        ),
        "nf_regnet_b0": _dcfg(
