...
class TResNet(nn.Module):
    def __init__(
        ...
        self.feature_info = [
            {"num_chs": self.planes, "reduction": 2, "module": ""},  # Not with S2D?
            {"num_chs": self.planes * (Bottleneck.expansion if v2 else 1), "reduction": 4, "module": "body.layer1"},
            {"num_chs": self.planes * 2 * (Bottleneck.expansion if v2 else 1), "reduction": 8, "module": "body.layer2"},
            {"num_chs": self.planes * 4 * Bottleneck.expansion, "reduction": 16, "module": "body.layer3"},
            {"num_chs": self.planes * 8 * Bottleneck.expansion, "reduction": 32, "module": "body.layer4"},
        ]
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            layers += [
                ConvNormAct(self.inplanes, planes * block.expansion, kernel_size=1, stride=1, apply_act=False, **dd)
            ]
    ...
    def group_matcher(self, coarse=False):
        matcher = {"stem": r"^body\.conv1", "blocks": r"^body\.layer(\d+)" if coarse else r"^body\.layer(\d+)\.(\d+)"}
        return matcher
    ...
    def forward_features(self, x):
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            x = self.body.conv1(x)
            x = checkpoint_seq(
                [self.body.layer1, self.body.layer2, self.body.layer3, self.body.layer4], x, flatten=True
            )
        else:
            x = self.body(x)
        return x
...
def _create_tresnet(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ...
        feature_cfg={"out_indices": (1, 2, 3, 4), "flatten_sequential": True},
        **kwargs,
    )
...
default_cfgs = generate_default_cfgs(
    {
        ...
        "tresnet_m.miil_in1k_448": _cfg(input_size=(3, 448, 448), pool_size=(14, 14), hf_hub_id="timm/"),
        "tresnet_l.miil_in1k_448": _cfg(input_size=(3, 448, 448), pool_size=(14, 14), hf_hub_id="timm/"),
        "tresnet_xl.miil_in1k_448": _cfg(input_size=(3, 448, 448), pool_size=(14, 14), hf_hub_id="timm/"),
...
def tresnet_l(pretrained=False, **kwargs) -> TResNet:
    model_args = {"layers": [4, 5, 18, 3], "width_factor": 1.2}
...
def tresnet_xl(pretrained=False, **kwargs) -> TResNet:
    model_args = {"layers": [4, 5, 24, 3], "width_factor": 1.3}
...
def tresnet_v2_l(pretrained=False, **kwargs) -> TResNet:
    model_args = {"layers": [3, 4, 23, 3], "width_factor": 1.0, "v2": True}
