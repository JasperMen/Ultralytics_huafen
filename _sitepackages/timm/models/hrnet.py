...
class HighResolutionModule(nn.Module):
    ...
    def _check_branches(self, num_branches, block_types, num_blocks, num_in_chs, num_channels):
        ...
        if num_branches != len(num_blocks):
            ...
            error_msg = f"num_branches({num_branches}) <> num_in_chs({len(num_in_chs)})"
        if error_msg:
...
class HighResolutionNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.feature_info = [{"num_chs": 64, "reduction": curr_stride, "module": "stem"}]
        for i, c in enumerate(self.head_channels if self.head_channels else num_channels):
            ...
            self.feature_info += [{"num_chs": c, "reduction": curr_stride, "module": f"stage{i + 1}"}]
    ...
    def group_matcher(self, coarse=False):
        matcher = {
            ...
            "block_types": r"^(?:layer|stage|transition)(\d+)"
            if coarse
            else [
                (r"^layer(\d+)\.(\d+)", None),
                (r"^stage(\d+)\.(\d+)", None),
...
class HighResolutionNetFeatures(HighResolutionNet):
    ...
    The design of HRNet makes it easy to grab feature maps, this class provides a simple wrapper to do so. It would be
    more complicated to use the FeatureNet helpers.

    The `feature_location=incre` allows grabbing increased channel count features using part of the classification head.
    If `feature_location=''` the default HRNet features are returned. First stem conv is used for stride 2 features.
    """
...
def _create_hrnet(variant, pretrained=False, cfg_variant=None, **model_kwargs):
    ...
    pretrained_strict = model_kwargs.pop(
        "pretrained_strict", not features_only and model_kwargs.get("head", "classification") == "classification"
    )
...
default_cfgs = generate_default_cfgs(
    {
        ...
        "hrnet_w18_ssld.paddle_in1k": _cfg(
            hf_hub_id="timm/", crop_pct=0.95, test_crop_pct=1.0, test_input_size=(3, 288, 288)
        ),
