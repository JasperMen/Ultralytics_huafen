...
class SEResNetBottleneck(Bottleneck):
    """ResNet bottleneck with a Squeeze-and-Excitation module. It follows Caffe implementation and uses `stride=stride`
    in `conv1` and not in `conv2` (the latter is used in the torchvision implementation of ResNet).
    """
...
class SENet(nn.Module):
    def __init__(
        ...
        """Parameters ---------- block (nn.Module): Bottleneck class. - For SENet154: SEBottleneck - For SE-ResNet
        models: SEResNetBottleneck - For SE-ResNeXt models: SEResNeXtBottleneck layers (list of ints): Number of
        residual blocks for 4 layers of the network (layer1...layer4). groups (int): Number of groups for the
        3x3 convolution in each bottleneck block. - For SENet154: 64 - For SE-ResNet models: 1 - For SE-ResNeXt
        models: 32 reduction (int): Reduction ratio for Squeeze-and-Excitation modules. - For all models: 16
        dropout_p (float or None): Drop probability for the Dropout layer. If `None` the Dropout layer is not
        used. - For SENet154: 0.2 - For SE-ResNet models: None - For SE-ResNeXt models: None inplanes (int):
        Number of input channels for layer1. - For SENet154: 128 - For SE-ResNet models: 64 - For SE-ResNeXt
        models: 64 input_3x3 (bool): If `True`, use three 3x3 convolutions instead of a single 7x7 convolution
        in layer0. - For SENet154: True - For SE-ResNet models: False - For SE-ResNeXt models: False
        downsample_kernel_size (int): Kernel size for downsampling convolutions in layer2, layer3 and layer4. -
        For SENet154: 3 - For SE-ResNet models: 1 - For SE-ResNeXt models: 1 downsample_padding (int): Padding
        for downsampling convolutions in layer2, layer3 and layer4. - For SENet154: 1 - For SE-ResNet models: 0
        - For SE-ResNeXt models: 0 num_classes (int): Number of outputs in `last_linear` layer. - For all
        models: 1000.
        """
        super().__init__()
        dd = {"device": device, "dtype": dtype}
        ...
        self.feature_info = [{"num_chs": inplanes, "reduction": 2, "module": "layer0"}]
        ...
        self.feature_info += [{"num_chs": 64 * block.expansion, "reduction": 4, "module": "layer1"}]
        ...
        self.feature_info += [{"num_chs": 128 * block.expansion, "reduction": 8, "module": "layer2"}]
        ...
        self.feature_info += [{"num_chs": 256 * block.expansion, "reduction": 16, "module": "layer3"}]
        ...
        self.feature_info += [{"num_chs": 512 * block.expansion, "reduction": 32, "module": "layer4"}]
    ...
    def group_matcher(self, coarse=False):
        matcher = {"stem": r"^layer0", "blocks": r"^layer(\d+)" if coarse else r"^layer(\d+)\.(\d+)"}
        return matcher
...
def legacy_seresnet18(pretrained=False, **kwargs) -> SENet:
    model_args = {"block": SEResNetBlock, "layers": [2, 2, 2, 2], "groups": 1, "reduction": 16}
...
def legacy_seresnet34(pretrained=False, **kwargs) -> SENet:
    model_args = {"block": SEResNetBlock, "layers": [3, 4, 6, 3], "groups": 1, "reduction": 16}
...
def legacy_seresnet50(pretrained=False, **kwargs) -> SENet:
    model_args = {"block": SEResNetBottleneck, "layers": [3, 4, 6, 3], "groups": 1, "reduction": 16}
...
def legacy_seresnet101(pretrained=False, **kwargs) -> SENet:
    model_args = {"block": SEResNetBottleneck, "layers": [3, 4, 23, 3], "groups": 1, "reduction": 16}
...
def legacy_seresnet152(pretrained=False, **kwargs) -> SENet:
    model_args = {"block": SEResNetBottleneck, "layers": [3, 8, 36, 3], "groups": 1, "reduction": 16}
...
def legacy_seresnext26_32x4d(pretrained=False, **kwargs) -> SENet:
    model_args = {"block": SEResNeXtBottleneck, "layers": [2, 2, 2, 2], "groups": 32, "reduction": 16}
...
def legacy_seresnext50_32x4d(pretrained=False, **kwargs) -> SENet:
    model_args = {"block": SEResNeXtBottleneck, "layers": [3, 4, 6, 3], "groups": 32, "reduction": 16}
...
def legacy_seresnext101_32x4d(pretrained=False, **kwargs) -> SENet:
    model_args = {"block": SEResNeXtBottleneck, "layers": [3, 4, 23, 3], "groups": 32, "reduction": 16}
