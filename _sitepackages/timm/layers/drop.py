...
def drop_block_2d(
    ...
    Args:
        ...
        couple_channels: If True, all channels share the same drop mask (per the original paper). If False, each channel
...
def calculate_drop_path_rates(
    ...
    Args:
        ...
        depths: Either a single int for total depth (per-block mode) or list of ints for depths per stage (stage-wise
            mode).
        stagewise: If True, use stage-wise pattern. If False, use per-block pattern. When depths is a list, stagewise
            defaults to True.
