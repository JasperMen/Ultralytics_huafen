...
def trunc_normal_(tensor, mean=0.0, std=1.0, a=-2.0, b=2.0):
    ...
    r"""Fills the input Tensor with values drawn from a truncated normal distribution. The values are effectively drawn
    from the normal distribution :math:`\mathcal{N}(\text{mean}, \text{std}^2)` with values outside :math:`[a, b]`
    redrawn until they are within the bounds. The method used for generating the random values works best when
...
def trunc_normal_tf_(tensor, mean=0.0, std=1.0, a=-2.0, b=2.0):
    ...
    NOTE: this 'tf' variant behaves closer to Tensorflow / JAX impl where the bounds [a, b] are applied when sampling
    the normal distribution with mean=0, std=1.0 and the result is subsequently scaled and shifted by the mean and std
    args.
...
def init_weight_vit(
    module: nn.Module, name: str, init_bias: float = 0.02, head_bias: float = 0.0, classifier_name: str = "head"
):
