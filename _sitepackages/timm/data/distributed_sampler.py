...
class OrderedDistributedSampler(Sampler):
    ...
    :class:`torch.nn.parallel.DistributedDataParallel`. In such case, each process can pass a DistributedSampler
    instance as a DataLoader sampler, and load a subset of the original dataset that is exclusive to it. .. note::
...
class RepeatAugSampler(Sampler):
    ...
    This sampler was taken from https://github.com/facebookresearch/deit/blob/0c4b8f60/samplers.py Used in Copyright (c)
    ...
    def __init__(
        ...
        self.epoch = 0
        self.num_samples = math.ceil(len(self.dataset) * num_repeats / self.num_replicas)
        self.total_size = self.num_samples * self.num_replicas
        ...
        if selected_round:
            ...
            self.num_selected_samples = math.ceil(len(self.dataset) / selected_ratio)
