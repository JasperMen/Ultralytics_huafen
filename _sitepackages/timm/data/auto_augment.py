...
def desaturate(img, factor, **_):
    factor = min(1.0, max(0.0, 1.0 - factor))
...
def _minmax_level_to_arg(level, _hparams, min_val=0.0, max_val=1.0, clamp=True):
...
class AugmentOp:
    ...
    def __call__(self, img):
        ...
        # setting magnitude_max overrides this to allow M > 10 (behavior closer to Google TF RA impl)
...
def auto_augment_transform(config_str: str, hparams: dict | None = None):
    """Create a AutoAugment transform.

    Args:
        config_str: String defining configuration of auto augmentation. Consists of multiple sections separated by
            dashes ('-'). The first section defines the AutoAugment policy (one of 'v0', 'v0r', 'original',
            'originalr'). While the remaining sections define other arguments
...
def rand_augment_transform(
    config_str: str,
    hparams: dict | None = None,
    transforms: str | dict | list | None = None,
):
    """Create a RandAugment transform.

    Args:
        config_str (str): String defining configuration of random augmentation. Consists of multiple sections separated
            by dashes ('-'). The first section defines the specific variant of rand augment (currently only 'rand'). The
            remaining sections, not order specific determine
...
class AugMixAugment:
    ...
    https://github.com/google-research/augmix/blob/master/imagenet.py From paper: 'AugMix: A Simple Data Processing
    Method to Improve Robustness and Uncertainty - https://arxiv.org/abs/1912.02781.
    """
...
def augment_and_mix_transform(config_str: str, hparams: dict | None = None):
    ...
    Args:
        ...
        'mstd' -  float std deviation of magnitude noise applied (default: 0) Ex 'augmix-m5-w4-d2' results in AugMix
            with severity 5, chain width 4, chain depth 2
