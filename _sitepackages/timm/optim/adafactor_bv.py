...
def _factored_dims(shape: tuple[int, ...], factored: bool, min_dim_size_to_factor: int) -> tuple[int, int] | None:
    ...
    This function returns a tuple with the two largest axes to reduce over. If no two dimensions have size >=
    min_dim_size_to_factor, return None.
...
class AdafactorBigVision(Optimizer):
    ...
    def __init__(
        ...
        momentum_dtype: str | torch.dtype = torch.bfloat16,
        eps: float | None = None,
            ...
            for p in group["params"]:
                ...
                if len(state) == 0:
                    ...
                    factored_dims = _factored_dims(
                        shape, factored=True, min_dim_size_to_factor=self.defaults["min_dim_size_to_factor"]
                    )
...
def _single_tensor_adafactor(
    params: list[Tensor],
    grads: list[Tensor],
    exp_avg_sq_rs: list[Tensor | None],
    exp_avg_sq_cs: list[Tensor | None],
    exp_avg_sqs: list[Tensor | None],
    ...
    *,
    ...
    weight_decay: float,
    momentum: float | None,
    momentum_dtype: str | torch.dtype,
        ...
        if weight_decay != 0:
            if unscaled_wd:
                ...
                if max_lr is None:
                    ...
                    param.mul_(1.0 - (lr / max_lr) * weight_decay)
            else:
                # match typical pytorch behavior for decoupled decay, eg adamw where wd is scaled by LR
                if max_lr is None:
                    param.mul_(1.0 - lr * weight_decay)
                else:
                    # corrected weight decay: scale by lr^2 / max_lr
                    param.mul_(1.0 - (lr**2 / max_lr) * weight_decay)
