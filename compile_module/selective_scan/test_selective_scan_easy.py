...
def selective_scan_easy(
    us, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=False, return_last_state=False, chunksize=64
):
    """# B: batch_size, G: groups, D: dim, N: state dim, L: seqlen us: B, G * D, L dts: B, G * D, L As: G * D, N Bs: B,
    G, N, L Cs: B, G, N, L Ds: G * D delta_bias: G * D # chunksize can be any as you like. But as the chunksize
    raises, hs may get None, as exp(sum(delta) A) is really small.
    """

    def selective_scan_chunk(us, dts, As, Bs, Cs, hprefix):
        """partial(h) / partial(t) = Ah + Bu; y = Ch + Du; => partial(h*exp(-At)) / partial(t) = Bu*exp(-At); => h_t =
        h_0 + sum_{0}_{t}_{Bu*exp(A(t-v)) dv}; => h_b = exp(A(dt_a + ... + dt_{b-1})) * (h_a +
        sum_{a}_{b-1}_{Bu*exp(-A(dt_a + ... + dt_i)) dt_i}); y_i = C_i*h_i + D*u_i.
        """
        """
...
class SelectiveScanEasy(torch.autograd.Function):
    ...
    def forward(
        ctx, us, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=False, return_last_state=False, chunksize=64
    ):
        has_D = Ds is not None
        dtype = torch.float32
    ...
    def backward(ctx, doys: torch.Tensor, *args):
        ...
        for i in chunks[::-1]:
            ...
            if DEBUG:
                print(
                    ...
                    (
                        torch.autograd.grad(
                            _oys,
                            tmp_fwd_dtBus_div_rAts_cumsum[chunks.index(i)],
                            doys,
                            create_graph=True,
                            allow_unused=True,
                        )[0]
                        - dhs_minus_prefix_div_rAts
                    )
                    .abs()
                    .sum(),
                )
            ...
            if DEBUG:
                d_dtBus_div_rAts_v1 = torch.autograd.grad(
                    hs_minus_prefix_div_rAts,
                    dtBus_div_rAts,
                    dhs_minus_prefix_div_rAts,
                    create_graph=True,
                    allow_unused=True,
                ...
                print(
                    ...
                    (
                        torch.autograd.grad(
                            _oys, tmp_fwd_dtBus_div_rAts[chunks.index(i)], doys, create_graph=True, allow_unused=True
                        )[0]
                        - d_dtBus_div_rAts
                    )
                    .abs()
            ...
            if DEBUG:
                print(
                    ...
                    (
                        torch.autograd.grad(
                            _oys, tmp_fwd_dtBus[chunks.index(i)], doys, create_graph=True, allow_unused=True
                        )[0]
                        - ddtBus
                    )
                    .abs()
            ...
            if DEBUG:
                ...
                print(
                    "ex.0",
                    (
                        torch.autograd.grad(tmp_b, tmp_a, tmp_c, create_graph=True, allow_unused=True)[0]
                        - rev_comsum(tmp_c)
                    )
                    .abs()
                    .sum(),
            ...
            if DEBUG:
                ...
                drAts_dtBus_div_rAts_ref = torch.autograd.grad(
                    dtBus_div_rAts, rAts, d_dtBus_div_rAts, create_graph=True, allow_unused=True
                )[0]
            ...
            if DEBUG:
                drAts_ref = torch.autograd.grad(
                    _oys, tmp_fwd_rAts[chunks.index(i)], doys, create_graph=True, allow_unused=True
                )[0]
                dAts_ref = torch.autograd.grad(
                    _oys, tmp_fwd_Ats[chunks.index(i)], doys, create_graph=True, allow_unused=True
                )[0]
        ...
        if DEBUG:
            print(
                "f",
                (torch.autograd.grad(_oys, dts, doys, create_graph=True, allow_unused=True)[0] - ddts).abs().sum(),
                flush=True,
            )
        ...
        if DEBUG:
            print(
                "f",
                (torch.autograd.grad(_oys, us, doys, create_graph=True, allow_unused=True)[0] - dus).abs().sum(),
                flush=True,
            )
            print(
                "f",
                (torch.autograd.grad(_oys, Bs, doys, create_graph=True, allow_unused=True)[0] - dBs).abs().sum(),
                flush=True,
            )
            print(
                "f",
                (torch.autograd.grad(_oys, Cs, doys, create_graph=True, allow_unused=True)[0] - dCs).abs().sum(),
                flush=True,
            )
            print(
                "f",
                (torch.autograd.grad(_oys, Ds, doys, create_graph=True, allow_unused=True)[0].view(-1) - dDs)
                .abs()
                .sum(),
                flush=True,
            )
            print(
                "f",
                (torch.autograd.grad(_oys, As, doys, create_graph=True, allow_unused=True)[0] - dAs).abs().sum(),
                flush=True,
            )
...
def selective_scan_easy_fwdbwd(
    u, delta, A, B, C, D, delta_bias=None, delta_softplus=None, return_last_state=False, chunksize=64
):
    mode = MODE
    if mode in ["fnDEBUG"]:
        outs = SelectiveScanEasy.forward(
            SelectiveScanEasy, u, delta, A, B, C, D, delta_bias, delta_softplus, return_last_state, chunksize
        )
...
def selective_scan_easyv3(
    ...
    def cut_chunk(us, dts, Bs, Cs, chunksize=chunksize):
        ...
        _pad = lambda x: torch.nn.functional.pad(x.view(B * H, L, -1), (0, 0, 0, to_pad, 0, 0)).view(
            B * H, NT, chunksize, x.shape[-1]
        )
        us, dts, Bs, Cs = _pad(us), _pad(dts), _pad(Bs), _pad(Cs)
        return us, dts, Bs, Cs

    def ss_chunk_h1y1(qs, ks, vs, ws=None, As=None, ts=None, dts=None, mask=None, scale=1):
        # C = n_chunks, M = B * H, E = B * H * C, T = L / C
        # MCTN, MCTN, MCTD; MCTND; HND, MCTD, MCTD;
        if ws is None:
            if ts is None:
                ts = torch.cumsum(dts * inv_ln2, dim=2)
            _ts = ts.view(-1, As.shape[0], *ts.shape[1:])[:, :, :, :, None, :]
            ws = torch.exp2(As[None, :, None, None, :, :] * _ts).flatten(0, 1)  # MCND
    ...
    def ss_chunk_h1y1_dk1(qs, ks, vs, ws=None, As=None, ts=None, dts=None, mask=None, scale=1):
        # C = n_chunks, M = B * H, E = B * H * C, T = L / C
        # MCTN, MCTN, MCTD; MCTND; HND, MCTD, MCTD;
        _M, _C, _T, N = qs.shape
        assert N == 1
        ...
        y1 = ws[:, :, :, 0, :] * torch.einsum(
            "mctr,mcrd->mctd", qs[:, :, :, None, 0] * ks[:, :, None, :, 0] * mask[None, None, :, :], v_div_w
        )
    ...
    _us, dts, Bs, Cs = cut_chunk(us, dts, Bs, Cs, chunksize=chunksize)
    ht1, y1, _ws, cws, q_mul_w = ss_chunk_h1y1(Cs, Bs, _us * dts, None, As, None, dts, mask=_mask_tril)
...
def selective_scan_easyv2_fwdbwd(
    u, delta, A, B, C, D, delta_bias=None, delta_softplus=None, return_last_state=False, chunksize=64
):
...
from ssmtriton import selective_scan_easyv3 as ss

selective_scan_easy = ss


# api to fit original mamba_ssm
def build_api_selective_scan(chunksize=64):
    def selective_scan_fn(u, delta, A, B, C, D, z=None, delta_bias=None, delta_softplus=None, return_last_state=False):
        assert z is None
...
def selective_scan_ref(
    ...
    """u: r(B D L) delta: r(B D L) A: c(D N) or r(D N) B: c(D N) or r(B N L) or r(B N 2L) or r(B G N L) or (B G N L) C:
    c(D N) or r(B N L) or r(B N 2L) or r(B G N L) or (B G N L) D: r(D) z: r(B D L) delta_bias: r(D), fp32.
...
@pytest.mark.parametrize("seqlen", [65, 128, 256, 512, 1024, 2048, 4096])
...
if __name__ == "__main__":
    test_selective_scan(True, True, 3, True, False, True, True, True, 1011, torch.float32, torch.float32, 64)
