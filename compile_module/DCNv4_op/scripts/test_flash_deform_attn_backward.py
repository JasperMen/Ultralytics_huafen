...
from functions import FlashDeformAttnFunction, MSDeformAttnFunction, ms_deform_attn_core_pytorch
...
def speed_test(func, args, inputs, name="Unknown"):
    ...
    print(f">>> {name: <10} finished {args.test_num} running, avg_time: {avg_time:.6f} ms")
    return avg_time


def check_forward_equal_with_pytorch_half():
    value = torch.rand(N, S, M, D).cuda() * 0.01
    (torch.rand(N, Lq, M, L, P, 2).cuda() * 2 - 1) / 10
    ...
    sampling_loc_attn = torch.cat(
        [
            sampling_locations.detach().reshape(N, Lq, M, L * P * 2),
            attention_weights_origin.detach().reshape(N, Lq, M, L * P),
        ],
        dim=-1,
    )

    attention_weights = torch.nn.functional.softmax(attention_weights_origin.flatten(-2, -1), dim=-1).unflatten(
        -1, (L, P)
    )
    ...
    output_pytorch = ms_deform_attn_core_pytorch(value1, shapes, sampling_locations, attention_weights)
    ...
    exp_time = speed_test(FlashMSDeformAttnFunction.apply, test_args, flash_dcn_fn_args, name="exp")
    exp_time_base = speed_test(MSDeformAttnFunction.apply, test_args, fn_args, name="exp")
