...
"""
    ...
    def emit(self, operation):
        ...
        warp_shape = [
            int(operation.tile_description.threadblock_shape[idx] / operation.tile_description.warp_count[idx])
            for idx in range(3)
        ]
...
def GenerateConv3dTensorOp(manifest, tile_descriptions, min_cc, align=128):
    ...
    for tile in tile_descriptions:
        for conv_kind in [ConvKind.Fprop, ConvKind.Dgrad, ConvKind.Wgrad]:
            if conv_kind == ConvKind.Fprop or (
                tile.math_instruction.element_accumulator in [DataType.f16, DataType.f32]
            ):
                output_types = (
                    [tile.math_instruction.element_a, tile.math_instruction.element_accumulator]
                    if DataTypeSize[tile.math_instruction.element_accumulator] == 32
                    else [
                        tile.math_instruction.element_accumulator,
                    ]
                )
                ...
                for output_type in output_types:
                    A = TensorDescription(
                        ...
                        int(align / DataTypeSize[tile.math_instruction.element_a]),
                    )
                    B = TensorDescription(
                        tile.math_instruction.element_b,
                        LayoutType.TensorNDHWC,
                        int(align / DataTypeSize[tile.math_instruction.element_b]),
                    )
                    C = TensorDescription(
                        output_type, LayoutType.TensorNDHWC, max(1, int(align / DataTypeSize[output_type]))
                    )

                    manifest.append(
                        Conv3dOperation(conv_kind, min_cc, tile, A, B, C, tile.math_instruction.element_accumulator)
                    )
...
class EmitConv3dConfigurationLibrary:
    def __init__(self, operation_path, configuration_name):
        ...
        self.configuration_path = os.path.join(operation_path, f"{configuration_name}.cu")
...
"""
    ...
    def __enter__(self):
        ...
        self.configuration_file.write(
            SubstituteTemplate(self.header_template, {"configuration_name": self.configuration_name})
        )
        self.operations = []
        return self
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        self.configuration_file.write(
            SubstituteTemplate(self.configuration_header, {"configuration_name": self.configuration_name})
        )
        ...
        for operation in self.operations:
            self.configuration_file.write(
                SubstituteTemplate(
                    self.configuration_instance,
                    {"configuration_name": self.configuration_name, "operation_name": operation.procedural_name()},
                )
            )
