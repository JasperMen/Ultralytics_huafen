...
"""
    ...
    def emit(self, configuration_name, operations):
        ...
        self.top_level_file.write(
            SubstituteTemplate(self.configuration_prototype_template, {"configuration_name": configuration_name})
        )

    def __exit__(self, exception_type, exception_value, traceback):
        self.top_level_file.write(
            SubstituteTemplate(self.entry_template, {"operation_name": OperationKindNames[self.kind]})
        )

        for configuration_name in self.configurations:
            self.top_level_file.write(
                SubstituteTemplate(self.configuration_template, {"configuration_name": configuration_name})
            )
...
class Manifest:
    def __init__(self, args):
        ...
        if args.operations == "all":
            ...
            operations_list = [OperationKind.Gemm, OperationKind.Conv2d, OperationKind.Conv3d]

            self.operations_enabled = [
                x for x in operations_list if OperationKindNames[x] in args.operations.split(",")
            ]
...
"""
    ...
    def get_kernel_filters(self, kernelListFile):
        if os.path.isfile(kernelListFile):
            with open(kernelListFile) as fileReader:
    ...
    def filter(self, operation):
        ...
        if operation.procedural_name() in self.operations_by_name:
            return False
    ...
    def append(self, operation):
        ...
        if self.filter(operation):
            ...
            if operation.operation_kind not in self.operations:
                self.operations[operation.operation_kind] = {}

            if configuration_name not in self.operations[operation.operation_kind]:
    ...
    def emit(self, target=GeneratorTarget.Library):
        ...
        with open(top_level_path, "w") as top_level_file:
            ...
            top_level_file.write(SubstituteTemplate(self.top_level_prologue, {"prototypes": "\n".join(prototypes)}))

            top_level_file.write(
                SubstituteTemplate(self.top_level_reserve, {"operation_count": str(self.operation_count)})
            )
...
""",
            ...
            manifest_file.writelines(
                "    {}\n".format(str(source_file.replace("\\", "/"))) for source_file in source_files
            )
            manifest_file.write(")")
