...
def CreateGemmPlanarComplexOperator(
        ...
        for conv_kind in conv_kinds:
            for iterator_algorithm in iterator_algorithms:
                ...
                if (iterator_algorithm == IteratorAlgorithm.Optimized) and (
                    (conv_kind == ConvKind.Fprop) or (conv_kind == ConvKind.Dgrad)
                ):
                    continue

                # strided support for Fprop (Analytic/Optimized), Dgrad (Analytic), and Wgrad (Analytic)
                new_operation = Conv3dOperation(
                    conv_kind,
                    iterator_algorithm,
                    tile.minimum_compute_capability,
                    tile,
...
def GenerateSM50_Simt(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [1, 1, 1], DataType.f32, DataType.f32, DataType.f32, OpcodeClass.Simt, MathOperation.multiply_add
        ),
        MathInstruction(
            [1, 1, 1], DataType.f64, DataType.f64, DataType.f64, OpcodeClass.Simt, MathOperation.multiply_add
        ),
    ]
...
def GenerateSM50_Simt_complex(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [1, 1, 1], DataType.f32, DataType.f32, DataType.f32, OpcodeClass.Simt, MathOperation.multiply_add_complex
        ),
    ]
...
def GenerateSM60_Simt(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [1, 1, 1], DataType.f16, DataType.f16, DataType.f16, OpcodeClass.Simt, MathOperation.multiply_add
        ),
    ]
...
def GenerateSM61_Simt(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [1, 1, 4], DataType.s8, DataType.s8, DataType.s32, OpcodeClass.Simt, MathOperation.multiply_add
        ),
    ]
...
def GenerateSM70_TensorOp_884(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [8, 8, 4], DataType.f16, DataType.f16, DataType.f32, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
        MathInstruction(
            [8, 8, 4], DataType.f16, DataType.f16, DataType.f16, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
    ]
...
def GenerateSM70_PlanarComplexTensorOp_884(manifest, args):
    ...
    for math_inst in math_instructions:
        ...
        if math_inst.element_a != math_inst.element_accumulator:
            ...
            CreateGemmPlanarComplexOperator(
                manifest, layouts, tile_descriptions, data_type_mixed, alignment_constraints, complex_transforms
            )
...
def GenerateSM70_WmmaTensorOp_161616(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [16, 16, 16], DataType.f16, DataType.f16, DataType.f32, OpcodeClass.WmmaTensorOp, MathOperation.multiply_add
        ),
        MathInstruction(
            [16, 16, 16], DataType.f16, DataType.f16, DataType.f16, OpcodeClass.WmmaTensorOp, MathOperation.multiply_add
        ),
    ]
...
def GenerateSM75_TensorOp_1688(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [16, 8, 8], DataType.f16, DataType.f16, DataType.f32, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
        MathInstruction(
            [16, 8, 8], DataType.f16, DataType.f16, DataType.f16, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
    ]
...
def GenerateSM75_TensorOp_8816_TN(manifest, args):
    ...
    for math_inst in math_instructions:
        ...
        if math_inst.element_a != math_inst.element_accumulator:
            ...
            operations += CreateConv2dOperator(
                manifest,
                conv_layout,
                tile_descriptions,
                data_type_mixed,
...
def GenerateSM75_TensorOp_8832_TN(manifest, args):
    ...
    for math_inst in math_instructions:
        ...
        if math_inst.element_a != math_inst.element_accumulator:
            ...
            for op in operations:
                if op.tile_description.threadblock_shape[1] >= 128 or op.tile_description.threadblock_shape[1] == 64:
                    op.C.alignment = 8
                else:
                    op.C.alignment = 4
...
def GenerateSM75_TensorOp_88128(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [8, 8, 128], DataType.b1, DataType.b1, DataType.s32, OpcodeClass.TensorOp, MathOperation.xor_popc
        ),
    ]
...
def GenerateSM75_WmmaTensorOp_161616(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [16, 16, 16], DataType.s8, DataType.s8, DataType.s32, OpcodeClass.WmmaTensorOp, MathOperation.multiply_add
        ),
    ]
...
def GenerateSM80_TensorOp_16816(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [16, 8, 16], DataType.f16, DataType.f16, DataType.f32, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
        MathInstruction(
            [16, 8, 16], DataType.f16, DataType.f16, DataType.f16, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
        MathInstruction(
            [16, 8, 16], DataType.bf16, DataType.bf16, DataType.f32, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
    ]
...
def GenerateSM80_SparseTensorOp_16832(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [16, 8, 32], DataType.f16, DataType.f16, DataType.f32, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
        MathInstruction(
            [16, 8, 32], DataType.f16, DataType.f16, DataType.f16, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
        MathInstruction(
            [16, 8, 32], DataType.bf16, DataType.bf16, DataType.f32, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
    ]
...
def GenerateSM80_SparseTensorOp_16864_TN(manifest, args):
    ...
    math_inst = MathInstruction(
        [16, 8, 64], DataType.s8, DataType.s8, DataType.s32, OpcodeClass.TensorOp, MathOperation.multiply_add_saturate
    )
    ...
    operations += CreateSparseGemmOperator(
        manifest,
        layouts,
        tile_descriptions,
        data_type_mixed,
...
def GenerateSM80_SparseTensorOp_168128_TN(manifest, args):
    ...
    math_inst = MathInstruction(
        [16, 8, 128], DataType.s4, DataType.s4, DataType.s32, OpcodeClass.TensorOp, MathOperation.multiply_add_saturate
    )
...
def GenerateSM80_TensorOp_168256(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [16, 8, 256], DataType.b1, DataType.b1, DataType.s32, OpcodeClass.TensorOp, MathOperation.xor_popc
        ),
    ]
...
def GenerateSM80_TensorOp_1688(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [16, 8, 8], DataType.tf32, DataType.tf32, DataType.f32, OpcodeClass.TensorOp, MathOperation.multiply_add
        )
    ]
...
def GenerateSM80_SparseTensorOp_16816_fast_math(manifest, args):
    ...
    math_instructions = [
        MathInstruction(
            [16, 8, 16], DataType.tf32, DataType.tf32, DataType.f32, OpcodeClass.TensorOp, MathOperation.multiply_add
        ),
    ]
...
def GenerateSM80_TensorOp_1688_complex(manifest, args):
    ...
    math_inst = MathInstruction(
        [16, 8, 8], DataType.f32, DataType.f32, DataType.f32, OpcodeClass.TensorOp, MathOperation.multiply_add_complex
    )
...
def GenerateSM80_TensorOp_884(manifest, args):
    ...
    math_inst = MathInstruction(
        [8, 8, 4], DataType.f64, DataType.f64, DataType.f64, OpcodeClass.TensorOp, MathOperation.multiply_add
    )
...
def GenerateSM80_TensorOp_884_complex(manifest, args):
    ...
    math_inst = MathInstruction(
        [8, 8, 4], DataType.f64, DataType.f64, DataType.f64, OpcodeClass.TensorOp, MathOperation.multiply_add_complex
    )
...
if __name__ == "__main__":
    ...
    if args.selected_kernel_list is not None and len(manifest.selected_kernels) > 0:
        with open(args.selected_kernel_list, "w") as file_writer:
            file_writer.writelines(f"{line}\n" for line in manifest.selected_kernels)
#
###################################################################################################
