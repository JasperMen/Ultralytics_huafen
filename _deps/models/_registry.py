...
def _deprecated_model_shim(deprecated_name: str, current_fn: Callable | None = None, current_tag: str = ""):
    def _fn(pretrained=False, **kwargs):
        ...
        current_name = f"{current_fn.__name__}.{current_tag}" if current_tag else current_fn.__name__
...
def register_model_deprecations(module_name: str, deprecation_map: dict[str, str | None]):
...
def list_models(
    ...
    if include_tags is None:
        # FIXME should this be default behavior? or default to include_tags=True?
...
def model_entrypoint(model_name: str, module_filter: str | None = None) -> Callable[..., Any]:
...
def is_model_in_modules(model_name: str, module_names: tuple[str, ...] | list[str] | set[str]) -> bool:
...
def get_pretrained_cfg(model_name: str, allow_unregistered: bool = True) -> PretrainedCfg | None:
