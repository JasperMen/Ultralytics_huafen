...
from timm.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, IMAGENET_DPN_MEAN, IMAGENET_DPN_STD
from timm.layers import BatchNormAct2d, ConvNormAct, create_classifier, create_conv2d, get_norm_act_layer
    ...
    def __init__(
        ...
        if self.has_proj:
            ...
            if self.key_stride == 2:
                self.c1x1_w_s2 = BnActConv2d(in_chs=in_chs, out_chs=num_1x1_c + 2 * inc, kernel_size=1, stride=2, **dd)
            else:
                self.c1x1_w_s1 = BnActConv2d(in_chs=in_chs, out_chs=num_1x1_c + 2 * inc, kernel_size=1, stride=1, **dd)
...
class DPN(nn.Module):
    def __init__(
        ...
        self.feature_info = [{"num_chs": num_init_features, "reduction": 2, "module": "features.conv1_1"}]
        ...
        for i in range(2, k_sec[0] + 1):
            ...
            in_chs += inc
        self.feature_info += [{"num_chs": in_chs, "reduction": 4, "module": f"features.conv2_{k_sec[0]}"}]
        ...
        for i in range(2, k_sec[1] + 1):
            ...
            in_chs += inc
        self.feature_info += [{"num_chs": in_chs, "reduction": 8, "module": f"features.conv3_{k_sec[1]}"}]
        ...
        for i in range(2, k_sec[2] + 1):
            ...
            in_chs += inc
        self.feature_info += [{"num_chs": in_chs, "reduction": 16, "module": f"features.conv4_{k_sec[2]}"}]
        ...
        for i in range(2, k_sec[3] + 1):
            ...
            in_chs += inc
        self.feature_info += [{"num_chs": in_chs, "reduction": 32, "module": f"features.conv5_{k_sec[3]}"}]
...
def _create_dpn(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ...
        variant,
        pretrained,
        feature_cfg={"feature_concat": True, "flatten_sequential": True},
        **kwargs,
    )
