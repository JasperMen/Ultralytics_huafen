...
from timm.layers import DropPath, Linear, SelectAdaptivePool2d, calculate_drop_path_rates, trunc_normal_
...
class MLPBlock(nn.Module):
    def __init__(
        ...
        if layer_scale_init_value > 0:
            self.layer_scale = nn.Parameter(layer_scale_init_value * torch.ones((dim), **dd), requires_grad=True)
        else:
...
class Block(nn.Module):
    def __init__(
        ...
        pconv_fw_type: str = "split_cat",
        use_merge: bool = True,
        merge_size: int | tuple[int, int] = 2,
        device=None,
        dtype=None,
    ):
        dd = {"device": device, "dtype": dtype}
...
class FasterNet(nn.Module):
    def __init__(
        ...
        for i in range(self.num_stages):
            ...
            self.feature_info += [{"num_chs": dim, "reduction": 2 ** (i + 2), "module": f"stages.{i}"}]
...
def fasternet_t0(pretrained: bool = False, **kwargs: Any) -> FasterNet:
    model_args = {"embed_dim": 40, "depths": (1, 2, 8, 2), "drop_path_rate": 0.0, "act_layer": nn.GELU}
...
def fasternet_t1(pretrained: bool = False, **kwargs: Any) -> FasterNet:
    model_args = {"embed_dim": 64, "depths": (1, 2, 8, 2), "drop_path_rate": 0.02, "act_layer": nn.GELU}
...
def fasternet_t2(pretrained: bool = False, **kwargs: Any) -> FasterNet:
    model_args = {"embed_dim": 96, "depths": (1, 2, 8, 2), "drop_path_rate": 0.05}
...
def fasternet_s(pretrained: bool = False, **kwargs: Any) -> FasterNet:
    model_args = {"embed_dim": 128, "depths": (1, 2, 13, 2), "drop_path_rate": 0.1}
...
def fasternet_m(pretrained: bool = False, **kwargs: Any) -> FasterNet:
    model_args = {"embed_dim": 144, "depths": (3, 4, 18, 3), "drop_path_rate": 0.2}
...
def fasternet_l(pretrained: bool = False, **kwargs: Any) -> FasterNet:
    model_args = {"embed_dim": 192, "depths": (3, 4, 18, 3), "drop_path_rate": 0.3}
