...
class RelPosAttention(nn.Module):
    ...
    def __init__(
        ...
        qkv_bias: bool = False,
        qk_norm: bool = False,
        rel_pos_cls: type[nn.Module] | None = None,
        attn_drop: float = 0.0,
        proj_drop: float = 0.0,
    ...
    def forward(self, x, shared_rel_pos: torch.Tensor | None = None):
        B, N, C = x.shape
...
class VisionTransformerRelPos(nn.Module):
    ...
    * layer-scale (residual branch gain) enabled
    ...
    def __init__(
        ...
        rel_pos_args = {"window_size": feat_size, "prefix_tokens": self.num_prefix_tokens}
...
def vit_relpos_base_patch32_plus_rpn_256(pretrained=False, **kwargs) -> VisionTransformerRelPos:
    ...
    model_args = {"patch_size": 32, "embed_dim": 896, "depth": 12, "num_heads": 14, "block_fn": ResPostRelPosBlock}
...
def vit_relpos_base_patch16_plus_240(pretrained=False, **kwargs) -> VisionTransformerRelPos:
    ...
    model_args = {"patch_size": 16, "embed_dim": 896, "depth": 12, "num_heads": 14}
    model = _create_vision_transformer_relpos(
...
def vit_relpos_small_patch16_224(pretrained=False, **kwargs) -> VisionTransformerRelPos:
    """ViT-Base (ViT-B/16) w/ relative log-coord position, no class token."""
    model_args = {"patch_size": 16, "embed_dim": 384, "depth": 12, "num_heads": 6, "qkv_bias": False, "fc_norm": True}
    model = _create_vision_transformer_relpos(
...
def vit_relpos_medium_patch16_224(pretrained=False, **kwargs) -> VisionTransformerRelPos:
    """ViT-Base (ViT-B/16) w/ relative log-coord position, no class token."""
    model_args = {"patch_size": 16, "embed_dim": 512, "depth": 12, "num_heads": 8, "qkv_bias": False, "fc_norm": True}
    model = _create_vision_transformer_relpos(
...
def vit_relpos_base_patch16_224(pretrained=False, **kwargs) -> VisionTransformerRelPos:
    """ViT-Base (ViT-B/16) w/ relative log-coord position, no class token."""
    model_args = {"patch_size": 16, "embed_dim": 768, "depth": 12, "num_heads": 12, "qkv_bias": False, "fc_norm": True}
    model = _create_vision_transformer_relpos(
...
def vit_relpos_base_patch16_clsgap_224(pretrained=False, **kwargs) -> VisionTransformerRelPos:
    """ViT-Base (ViT-B/16) w/ relative log-coord position, class token present NOTE this config is a bit of a mistake,
    class token was enabled but global avg-pool w/ fc-norm was not disabled Leaving here for comparisons w/ a future
    re-train as it performs quite well.
    """
    model_args = {
        "patch_size": 16,
        "embed_dim": 768,
