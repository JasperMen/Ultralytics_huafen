...
def _dct_kernel_type_2(
    ...
    k = (
        torch.tensor(-1j, device=device, dtype=torch.complex64)
        * torch.pi
...
class CSATv2(nn.Module):
    ...
    A hybrid architecture that processes images in the DCT frequency domain with ConvNeXt-style blocks and transformer
    attention.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.feature_info = [{"num_chs": dims[0], "reduction": 8, "module": "stem_dct"}]
        reduction = 8
        for i, dim in enumerate(dims):
            if i > 0:
                reduction *= 2
            self.feature_info.append({"num_chs": dim, "reduction": reduction, "module": f"stages.{i}"})
        ...
        for depth, t_depth in zip(depths, transformer_depths):
            ...
            dp_rates += [next(dp_iter) if transformer_drop_path else 0.0 for _ in range(t_depth)]
        ...
        for i, (dim, depth, t_depth) in enumerate(zip(dims, depths, transformer_depths)):
            layers = (
                ...
                # Conv blocks
                [
                    Block(dim=dim, drop_path=next(dp_iter), ls_init_value=ls_init_value, **dd)
                    for _ in range(depth - t_depth)
                ]
                +
                # Transformer blocks at end of stage
                [
                    TransformerBlock(inp=dim, oup=dim, drop_path=next(dp_iter), ls_init_value=ls_init_value, **dd)
...
def checkpoint_filter_fn(state_dict: dict, model: nn.Module) -> dict:
    ...
    for k, v in state_dict.items():
        ...
        k = k.replace(".Y_Conv.", ".conv_y.").replace(".Cb_Conv.", ".conv_cb.").replace(".Cr_Conv.", ".conv_cr.")
        ...
        if ".attention.attention." in k:
            k = (
                k.replace(".attention.attention.attn.to_qkv.", ".attn.attn.qkv.")
...
def _create_csatv2(variant: str, pretrained: bool = False, **kwargs) -> CSATv2:
    ...
    return build_model_with_cfg(
        ...
        feature_cfg={"out_indices": out_indices, "flatten_sequential": True},
...
def csatv2_21m(pretrained: bool = False, **kwargs) -> CSATv2:
    ...
    model_args = {"dims": (48, 96, 224, 448), "depths": (3, 3, 10, 8), "transformer_depths": (0, 0, 4, 3)}
