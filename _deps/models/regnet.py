...
class RegNetCfg:
    ...
    stem_width: int = 32
    downsample: str | None = "conv1x1"
    linear_out: bool = False
    preact: bool = False
    num_features: int = 0
    act_layer: str | Callable = "relu"
    norm_layer: str | Callable = "batchnorm"
...
def adjust_widths_groups_comp(
    widths: list[int], bottle_ratios: list[float], groups: list[int], min_ratio: float = 0.0
...
def generate_regnet(
    width_slope: float, width_initial: int, width_mult: float, depth: int, group_size: int, quant: int = 8
    ...
    num_stages, _max_stage = len(torch.unique(widths)), int(width_exps.max().item()) + 1
...
class Bottleneck(nn.Module):
    ...
    This is almost exactly the same as a ResNet Bottleneck. The main difference is the SE block is moved from after
    conv3 to after conv2. Otherwise, it's just redefining the arguments for groups/bottleneck channels.
    """
    ...
    def __init__(
        ...
        bottleneck_chs = round(out_chs * bottle_ratio)
        groups = bottleneck_chs // group_size

        cargs = {"act_layer": act_layer, "norm_layer": norm_layer}
        ...
        if se_ratio:
            se_channels = round(in_chs * se_ratio)
...
class RegNet(nn.Module):
    ...
    Paper: https://arxiv.org/abs/2003.13678 Original Impl:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i, stage_args in enumerate(per_stage_args):
            ...
            self.feature_info += [{"num_chs": prev_width, "reduction": curr_stride, "module": stage_name}]
    ...
    def _get_stage_args(
        self, cfg: RegNetCfg, default_stride: int = 2, output_stride: int = 32, drop_path_rate: float = 0.0
        ...
        per_stage_args = [
            dict(zip(arg_names, params))
            for params in zip(stage_widths, stage_strides, stage_dilations, stage_depths, stage_br, stage_gs, stage_dpr)
        ]
        common_args = {
...
def _filter_fn(state_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    if "classy_state_dict" in state_dict:
        ...
        for k, v in state_dict["trunk"].items():
            ...
            k = re.sub(
                r"^_feature_blocks.res\d.block(\d)-(\d+)", lambda x: f"s{int(x.group(1))}.b{int(x.group(2)) + 1}", k
            )
    ...
    if "stem.0.weight" in state_dict:
        ...
        for k, v in state_dict.items():
            ...
            k = re.sub(
                r"trunk_output.block(\d)\.block(\d+)\-(\d+)", lambda x: f"s{int(x.group(1))}.b{int(x.group(3)) + 1}", k
            )
            for s, r in replaces:
                k = k.replace(s, r)
...
def _create_regnet(variant: str, pretrained: bool, **kwargs) -> RegNet:
    ...
    return build_model_with_cfg(
        RegNet, variant, pretrained, model_cfg=model_cfgs[variant], pretrained_filter_fn=_filter_fn, **kwargs
    )
...
default_cfgs = generate_default_cfgs(
    {
        ...
        # timm in12k pretrain
        "regnety_120.sw_in12k": _cfg(hf_hub_id="timm/", num_classes=11821),
        "regnety_160.sw_in12k": _cfg(hf_hub_id="timm/", num_classes=11821),
        ...
        "regnetx_004_tv.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_x_400mf-62229a5f.pth"
        ),
        "regnetx_008.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_x_800mf-94a99ebd.pth"
        ),
        "regnetx_016.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_x_1_6gf-a12f2b72.pth"
        ),
        "regnetx_032.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_x_3_2gf-7071aa85.pth"
        ),
        "regnetx_080.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_x_8gf-2b70d774.pth"
        ),
        "regnetx_160.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_x_16gf-ba3796d7.pth"
        ),
        "regnetx_320.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_x_32gf-6eb8fdc6.pth"
        ),
        "regnety_004.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_y_400mf-e6988f5f.pth"
        ),
        "regnety_008_tv.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_y_800mf-58fc7688.pth"
        ),
        "regnety_016.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_y_1_6gf-0d7bc02a.pth"
        ),
        "regnety_032.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_y_3_2gf-9180c971.pth"
        ),
        "regnety_080_tv.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_y_8gf-dc2b1b54.pth"
        ),
        "regnety_160.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_y_16gf-3e4a00f9.pth"
        ),
        "regnety_320.tv2_in1k": _cfgtv2(
            hf_hub_id="timm/", url="https://download.pytorch.org/models/regnet_y_32gf-8db6d4b5.pth"
        ),
...
def regnetz_005(pretrained: bool = False, **kwargs) -> RegNet:
    """RegNetZ-500MF NOTE: config found in
    https://github.com/facebookresearch/ClassyVision/blob/main/classy_vision/models/regnet.py but it's not clear it
    is equivalent to paper model as not detailed in the paper.
    """
...
def regnetz_040(pretrained: bool = False, **kwargs) -> RegNet:
    """RegNetZ-4.0GF NOTE: config found in
    https://github.com/facebookresearch/ClassyVision/blob/main/classy_vision/models/regnet.py but it's not clear it
    is equivalent to paper model as not detailed in the paper.
    """
