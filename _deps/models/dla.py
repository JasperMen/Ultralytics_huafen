...
class DlaBasic(nn.Module):
    ...
    def forward(self, x, shortcut: torch.Tensor | None = None, children: list[torch.Tensor] | None = None):
        if shortcut is None:
            shortcut = x
...
class DlaTree(nn.Module):
    def __init__(
        ...
        if levels == 1:
            ...
            cargs.update({"root_kernel_size": root_kernel_size, "root_shortcut": root_shortcut})
            self.tree1 = DlaTree(
                levels - 1,
                block,
                in_channels,
...
class DLA(nn.Module):
    def __init__(
        ...
        self.feature_info = [
            {"num_chs": channels[0], "reduction": 1, "module": "level0"},  # rare to have a meaningful stride 1 level
    ...
    def _make_conv_level(
        self, inplanes: int, planes: int, convs: int, stride: int = 1, dilation: int = 1, device=None, dtype=None
    ):
        dd = {"device": device, "dtype": dtype}
        modules = []
...
def _create_dla(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ...
        feature_cfg={"out_indices": (1, 2, 3, 4, 5)},
        **kwargs,
    )
...
def dla34(pretrained=False, **kwargs) -> DLA:  # DLA-34
    model_args = {"levels": [1, 1, 1, 2, 2, 1], "channels": [16, 32, 64, 128, 256, 512], "block": DlaBasic}
    return _create_dla("dla34", pretrained, **dict(model_args, **kwargs))


@register_model
def dla46_c(pretrained=False, **kwargs) -> DLA:  # DLA-46-C
    model_args = {"levels": [1, 1, 1, 2, 2, 1], "channels": [16, 32, 64, 64, 128, 256], "block": DlaBottleneck}
...
def dla60(pretrained=False, **kwargs) -> DLA:  # DLA-60
    model_args = {"levels": [1, 1, 1, 2, 3, 1], "channels": [16, 32, 128, 256, 512, 1024], "block": DlaBottleneck}
