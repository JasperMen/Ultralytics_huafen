...
__all__ = ["ByoBlockCfg", "ByoModelCfg", "ByobNet", "create_block", "create_byob_stem"]
...
class ByoBlockCfg:
    ...
    gs: int | Callable | None = None  # group-size of blocks in stage, conv is depthwise if gs == 1
...
class ByoModelCfg:
    ...
    blocks: tuple[ByoBlockCfg | tuple[ByoBlockCfg, ...], ...]
    downsample: str = "conv1x1"
    stem_type: str = "3x3"
    stem_pool: str | None = "maxpool"
    stem_chs: int | list[int] | tuple[int, ...] = 32
    ...
    # Head config
    head_hidden_size: int | None = None  # feat dim of MLP head or AttentionPool output
...
def _rep_vgg_bcfg(
    d: tuple[int, ...] = (4, 6, 16, 1), wf: tuple[float, ...] = (1.0, 1.0, 1.0, 1.0), groups: int = 0
) -> tuple[ByoBlockCfg, ...]:
...
def _mobileone_bcfg(
    ...
    wf: tuple[float, ...] = (1.0, 1.0, 1.0, 1.0),
    se_blocks: tuple[int, ...] = (),
    ...
    for d, c, w, se in zip(d, c, wf, se_blocks):
        ...
        for i in range(d):
            out_c = c * w
            bk = {"num_conv_branches": num_conv_branches}
            ak = {}
            if i >= d - se:
                ak["attn_layer"] = "se"
            scfg += [ByoBlockCfg(type="one", d=1, c=prev_c, gs=1, block_kwargs=bk, **ak)]  # depthwise block
            scfg += [
                ByoBlockCfg(type="one", d=1, c=out_c, gs=0, block_kwargs=dict(kernel_size=1, **bk), **ak)
            ]  # pointwise block
            prev_c = out_c
        bcfg += [scfg]
    return bcfg
...
def expand_blocks_cfg(stage_blocks_cfg: ByoBlockCfg | Sequence[ByoBlockCfg]) -> list[ByoBlockCfg]:
...
class LayerFn:
    ...
    self_attn: type[nn.Module] | None = None
...
class DarkBlock(nn.Module):
    ...
    The GE-Net impl included a 1x1 + 3x3 block in their search space. It was not used in the feature models. This block
    is pretty much a DarkNet block (also DenseNet) hence the name. Neither DarkNet or DenseNet uses strides within the
    block (external 3x3 or maxpool downsampling is done in front of the block repeats).

    If one does want to use a lot of these blocks w/ stride, I'd recommend using the EdgeBlock (3x3 /w stride + 1x1) for
    more optimal compute.
    """
...
class EdgeBlock(nn.Module):
    ...
    A two layer block like DarkBlock, but with the order of the 3x3 and 1x1 convs reversed. Very similar to the
    EfficientNet Edge-Residual block but this block it ends with activations, is intended to be used with either
    expansion or bottleneck contraction, and can use DW/group/non-grouped convs.
...
class SelfAttnBlock(nn.Module):
    ...
    def __init__(
        ...
        opt_kwargs = {} if feat_size is None else {"feat_size": feat_size}
...
def create_block(block: str | nn.Module, **kwargs):
...
class Stem(nn.Sequential):
    def __init__(
        self,
        in_chs: int,
        out_chs: int | list[int] | tuple[int, ...],
        kernel_size: int = 3,
        stride: int = 4,
        pool: str = "maxpool",
        num_rep: int = 3,
        ...
        for i, (ch, s, na) in enumerate(zip(stem_chs, stem_strides, stem_norm_acts)):
            ...
            if i > 0 and s > 1:
                ...
                self.feature_info.append(
                    {"num_chs": prev_chs, "reduction": curr_stride, "module": prev_feat, "stage": 0}
                )
        ...
        if pool:
            ...
            last_feat_idx = i
            self.feature_info.append({"num_chs": prev_chs, "reduction": curr_stride, "module": prev_feat, "stage": 0})
            if pool == "max2":
    ...
    def forward_intermediates(self, x) -> tuple[torch.Tensor, torch.Tensor | None]:
...
def create_byob_stem(
    ...
    if isinstance(stem, Stem):
        ...
        feature_info = [{"num_chs": out_chs, "reduction": 2, "module": feat_prefix, "stage": 0}]
...
def override_kwargs(block_kwargs, model_kwargs):
    ...
    NOTE: kwargs are NOT merged across levels, block_kwargs will fully replace model_kwargs for the block if set to
...
def update_block_kwargs(
    ...
    if self_attn_set or block_cfg.self_attn_kwargs is not None:
        ...
        if self_attn_set and not block_cfg.self_attn_layer:  # attn_layer == ''
            ...
            self_attn_layer = (
                partial(get_attn(self_attn_layer), **self_attn_kwargs) if self_attn_layer is not None else None
            )
...
def drop_blocks(
    ...
    DropBlock is applied to the last two stages only, following common practice. The block_size specifies the size for
    the final stage; the second-to-last stage uses a larger block size scaled to account for 2x larger feature maps.

    Args:
        drop_prob: Drop probability for DropBlock.
        block_size: Block size for the final stage. Second-to-last stage uses `block_size * 2 - 1` to scale with feature
            map size.
...
def create_byob_stages(
    ...
    layers: LayerFn | None = None,
    block_kwargs_fn: Callable | None = update_block_kwargs,
    device=None,
    dtype=None,
):
    layers = layers or LayerFn()
    ...
    for stage_idx, stage_block_cfgs in enumerate(block_cfgs):
        ...
        for block_idx, block_cfg in enumerate(stage_block_cfgs):
            ...
            block_kwargs = {  # Blocks used in this model must accept these arguments
                "in_chs": prev_chs,
                "out_chs": out_chs,
...
class ByobNet(nn.Module):
    ...
    A flexible network backbone that allows building model stem + blocks via dataclass cfg definition w/ factory
    functions for module instantiation.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: Extra kwargs overlaid onto cfg.
        """
        super().__init__()
        dd = {"device": device, "dtype": dtype}
        ...
        if isinstance(cfg.stem_chs, (list, tuple)):
            stem_chs = [round(c * cfg.width_factor) for c in cfg.stem_chs]
        else:
            stem_chs = round((cfg.stem_chs or cfg.blocks[0].c) * cfg.width_factor)
        ...
        if cfg.num_features:
            self.num_features = round(cfg.width_factor * cfg.num_features)
            self.final_conv = stage_layers.conv_norm_act(prev_chs, self.num_features, 1, **dd)
        else:
            self.num_features = prev_chs
            self.final_conv = nn.Identity()
        self.feature_info += [
            {"num_chs": self.num_features, "reduction": reduction, "module": "final_conv", "stage": len(self.stages)}
        ]
    ...
    def group_matcher(self, coarse: bool = False) -> dict[str, Any]:
        ...
        matcher = {
            "stem": r"^stem",
            "blocks": [(r"^stages\.(\d+)" if coarse else r"^stages\.(\d+)\.(\d+)", None), (r"^final_conv", (99999,))],
        }
        return matcher
...
model_cfgs = {
    ...
    "repvgg_d2se": ByoModelCfg(
        blocks=_rep_vgg_bcfg(d=(8, 14, 24, 1), wf=(2.5, 2.5, 2.5, 5.0)),
        stem_type="rep",
        stem_chs=64,
        attn_layer="se",
        attn_kwargs={"rd_ratio": 0.0625, "rd_divisor": 1},
    ),
    ...
    "resnet61q": ByoModelCfg(
        blocks=(
            ByoBlockCfg(type="edge", d=1, c=256, s=1, gs=0, br=1.0, block_kwargs={}),
            ByoBlockCfg(type="bottle", d=4, c=512, s=2, gs=32, br=0.25),
            ByoBlockCfg(type="bottle", d=6, c=1536, s=2, gs=32, br=0.25),
            ByoBlockCfg(type="bottle", d=4, c=1536, s=2, gs=1, br=1.0),
        ),
    ...
    "regnetz_b16": ByoModelCfg(
        ...
        block_kwargs={"bottle_in": True, "linear_out": True},
    ),
    ...
    "mobileone_s0": ByoModelCfg(
        blocks=_mobileone_bcfg(wf=(0.75, 1.0, 1.0, 2.0), num_conv_branches=4),
        stem_type="one",
        stem_chs=48,
    ),
...
def checkpoint_filter_fn(state_dict: dict[str, torch.Tensor], model: ByobNet):
...
default_cfgs = generate_default_cfgs(
    {
        ...
        "repvgg_a0.rvgg_in1k": _cfg(
            hf_hub_id="timm/", first_conv=("stem.conv_kxk.conv", "stem.conv_1x1.conv"), license="mit"
        ),
...
def gernet_l(pretrained=False, **kwargs) -> ByobNet:
    """GEResNet-Large (GENet-Large from official impl) `Neural Architecture Design for GPU-Efficient Networks` -
...
def gernet_m(pretrained=False, **kwargs) -> ByobNet:
    """GEResNet-Medium (GENet-Normal from official impl) `Neural Architecture Design for GPU-Efficient Networks` -
    https://arxiv.org/abs/2006.14090.
    """
...
def gernet_s(pretrained=False, **kwargs) -> ByobNet:
    """EResNet-Small (GENet-Small from official impl) `Neural Architecture Design for GPU-Efficient Networks` -
    https://arxiv.org/abs/2006.14090.
    """
...
def repvgg_a0(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-A0 `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
...
def repvgg_a1(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-A1 `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
...
def repvgg_a2(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-A2 `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
...
def repvgg_b0(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-B0 `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
...
def repvgg_b1(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-B1 `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
...
def repvgg_b1g4(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-B1g4 `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
...
def repvgg_b2(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-B2 `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
...
def repvgg_b2g4(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-B2g4 `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
...
def repvgg_b3(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-B3 `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
...
def repvgg_b3g4(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-B3g4 `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
...
def repvgg_d2se(pretrained=False, **kwargs) -> ByobNet:
    """RepVGG-D2se `Making VGG-style ConvNets Great Again` - https://arxiv.org/abs/2101.03697.
    """
