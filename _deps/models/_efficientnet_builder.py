...
from timm.layers import CondConv2d, LayerType, get_act_layer, get_attn, get_condconv_initializer, make_divisible
...
_BN_ARGS_TF = {"momentum": BN_MOMENTUM_TF_DEFAULT, "eps": BN_EPS_TF_DEFAULT}
...
def _decode_block_str(block_str):
    ...
    Gets a list of block arg (dicts) through a string notation of arguments. E.g. ir_r2_k3_s2_e1_i32_o16_se0.25_noskip

    All args can exist in any order with the exception of the leading string which is assumed to indicate the block
    type.

    leading string - block type ( ir = InvertedResidual, ds = DepthwiseSep, dsa = DeptwhiseSep with pw act, cn =
    ConvBnAct) r - number of repeat blocks, k - kernel size, s - strides (1-9), e - expansion ratio, c - output
    channels, se - squeeze/excitation ratio n - activation fn ('re', 'r6', 'hs', or 'sw')
    ...
    elif block_type == "mha" or block_type == "mqa":
...
def _scale_stage_depth(stack_args, repeats, depth_multiplier=1.0, depth_trunc="ceil"):
    """Per-stage depth scaling Scales the block repeats in each stage. This depth scaling impl maintains compatibility
    with the EfficientNet scaling method, while allowing sensible scaling for other models that may have multiple
    block arg definitions in each stage.
    """
    ...
    if depth_trunc == "round":
        ...
        num_repeat_scaled = math.ceil(num_repeat * depth_multiplier)
...
class EfficientNetBuilder:
    ...
    https://github.com/tensorflow/tpu/blob/master/models/official/mnasnet/mnasnet_models.py and
    ...
    def _make_block(self, ba, block_idx, block_count):
        ...
        if bt == "ir":
            _log_info_if(f"  InvertedResidual {block_idx}, Args: {ba!s}", self.verbose)
            block = CondConvResidual(**ba) if ba.get("num_experts", 0) else InvertedResidual(**ba)
        elif bt == "ds" or bt == "dsa":
            _log_info_if(f"  DepthwiseSeparable {block_idx}, Args: {ba!s}", self.verbose)
            block = DepthwiseSeparableConv(**ba)
        elif bt == "er":
            _log_info_if(f"  EdgeResidual {block_idx}, Args: {ba!s}", self.verbose)
            block = EdgeResidual(**ba)
        elif bt == "cn":
            _log_info_if(f"  ConvBnAct {block_idx}, Args: {ba!s}", self.verbose)
            block = ConvBnAct(**ba)
        elif bt == "uir":
            _log_info_if(f"  UniversalInvertedResidual {block_idx}, Args: {ba!s}", self.verbose)
            block = UniversalInvertedResidual(**ba, layer_scale_init_value=self.layer_scale_init_value)
        elif bt == "mqa":
            _log_info_if(f"  MobileMultiQueryAttention {block_idx}, Args: {ba!s}", self.verbose)
            block = MobileAttention(**ba, use_multi_query=True, layer_scale_init_value=self.layer_scale_init_value)
        elif bt == "mha":
            _log_info_if(f"  MobileMultiHeadAttention {block_idx}, Args: {ba!s}", self.verbose)
            block = MobileAttention(**ba, layer_scale_init_value=self.layer_scale_init_value)
        else:
            assert False, f"Unknown block type ({bt}) while building model."
    ...
    def __call__(self, in_chs, model_block_args):
        ...
        Args:
            ...
            model_block_args: A list of lists, outer list defines stages, inner list contains strings defining block
                configuration(s).
        ...
        if model_block_args[0][0]["stride"] > 1:
            ...
            feature_info = {"module": "bn1", "num_chs": in_chs, "stage": 0, "reduction": current_stride}
            self.features.append(feature_info)

        # outer list of block_args defines the stacks
        space2depth = 0
        for stack_idx, stack_args in enumerate(model_block_args):
            stack_idx + 1 == len(model_block_args)
            ...
            for block_idx, block_args in enumerate(stack_args):
                ...
                if last_block:
                    ...
                    extract_features = (
                        next_stack_idx >= len(model_block_args) or model_block_args[next_stack_idx][0]["stride"] > 1
                    )
