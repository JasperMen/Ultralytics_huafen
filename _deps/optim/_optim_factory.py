...
from ._types import OptimizerCallable, OptimType, ParamsT
...
class OptimizerRegistry:
    ...
    This class provides a central registry for optimizer configurations and handles their instantiation with appropriate
    ...
    def list_optimizers(
        self, filter: str | list[str] = "", exclude_filters: list[str] | None = None, with_description: bool = False
    ...
    def get_optimizer_class(
        self,
        name_or_info: str | OptimInfo,
        bind_defaults: bool = True,
    ) -> OptimType | OptimizerCallable:
    ...
    def create_optimizer(
        self,
        model_or_params: nn.Module | ParamsT,
        opt: str,
        lr: float | None = None,
        ...
        layer_decay_min_scale: float | None = None,
        layer_decay_no_opt_scale: float | None = None,
        param_group_fn: Callable[[nn.Module], ParamsT] | None = None,
        **kwargs: Any,
        ...
        Args:
            ...
            fallback_no_weight_decay: If True, params in no_weight_decay list will use fallback optimizer (e.g., AdamW
                for Muon)
...
def _register_lamb_lars(registry: OptimizerRegistry) -> None:
    ...
    lamb_lars_optimizers = [
        ...
        OptimInfo(name="lars", opt_class=Lars, description="Layer-wise Adaptive Rate Scaling", has_momentum=True),
        OptimInfo(
            name="larc",
            opt_class=Lars,
...
def _register_cautious_optimizers(registry: OptimizerRegistry) -> None:
    cautious_optimizers = [
        OptimInfo(name="cadafactor", opt_class=Adafactor, description="Cautious Adafactor", defaults={"caution": True}),
        OptimInfo(
            name="cadafactorbv",
            opt_class=AdafactorBigVision,
        ...
        OptimInfo(name="cadopt", opt_class=Adopt, description="Cautious Adopt", defaults={"caution": True}),
        OptimInfo(
            name="cadan",
            opt_class=Adan,
        ...
        OptimInfo(
            name="clamb", opt_class=Lamb, description="Cautious LAMB", has_betas=True, defaults={"caution": True}
        ),
        OptimInfo(
            name="clambw",
            opt_class=Lamb,
        ...
        OptimInfo(
            name="claprop", opt_class=LaProp, description="Cautious LaProp", has_betas=True, defaults={"caution": True}
        ),
        OptimInfo(
            name="clion",
            opt_class=Lion,
        ...
        OptimInfo(
            name="cmars", opt_class=Mars, description="Cautious MARS", has_betas=True, defaults={"caution": True}
        ),
        OptimInfo(
            name="cnadamw", opt_class=NAdamW, description="Cautious NAdamW", has_betas=True, defaults={"caution": True}
        ),
        OptimInfo(
            name="crmsproptf",
            opt_class=RMSpropTF,
...
def _register_other_optimizers(registry: OptimizerRegistry) -> None:
    ...
    other_optimizers = [
        ...
        OptimInfo(
            name="madgrad", opt_class=MADGRAD, description="Momentum-based Adaptive gradient method", has_momentum=True
        ),
        OptimInfo(
            name="madgradw",
            opt_class=MADGRAD,
...
def _register_bnb_optimizers(registry: OptimizerRegistry) -> None:
    ...
    bnb_optimizers = [
        ...
        OptimInfo(name="bnbadam", opt_class="bitsandbytes.optim.Adam", description="bitsandbytes Adam", has_betas=True),
        OptimInfo(
            name="bnbadam8bit",
        ...
        OptimInfo(
            name="bnbadamw", opt_class="bitsandbytes.optim.AdamW", description="bitsandbytes AdamW", has_betas=True
        ),
        OptimInfo(
            name="bnbadamw8bit",
        ...
        OptimInfo("bnblion", "bitsandbytes.optim.Lion", description="bitsandbytes Lion", has_eps=False, has_betas=True),
        OptimInfo(
            "bnblion8bit",
...
def list_optimizers(
    ...
    List all registered optimizers, with optional filtering using wildcard patterns. Optimizers can be filtered using
    include and exclude patterns, and can optionally return descriptions with each optimizer name.

    Args:
        filter: Wildcard style filter string or list of filter strings (e.g., 'adam*' for all Adam variants, or
            ['adam*', '*8bit'] for Adam variants and 8-bit optimizers). Empty string means no filtering.
        exclude_filters: Optional list of wildcard patterns to exclude. For example, ['*8bit', 'fused*'] would exclude
            8-bit and fused implementations.
        with_description: If True, returns tuples of (name, description) instead of just names. Descriptions provide
            brief explanations of optimizer characteristics.
...
def get_optimizer_class(
    ...
    Retrieves the optimizer class or a partial function with default arguments bound. This allows direct instantiation
    of optimizers with their default configurations without going through the full factory.

    Args:
        name: Name of the optimizer to retrieve (e.g., 'adam', 'sgd')
        bind_defaults: If True, returns a partial function with default arguments from OptimInfo bound. If False,
            returns the raw optimizer class.
...
def create_optimizer_v2(
    ...
    **kwargs: Any,
    ...
    Creates and configures an optimizer with appropriate parameter groups and settings. Supports automatic parameter
    group creation for weight decay and layer-wise learning rates, as well as custom parameter grouping.

    Args:
        model_or_params: A PyTorch model or an iterable of parameters/parameter groups. If a model is provided,
            parameters will be automatically extracted and grouped based on the other arguments.
        opt: Name of the optimizer to create (e.g., 'adam', 'adamw', 'sgd'). Use list_optimizers() to see available
            options.
        ...
        momentum: Momentum factor for optimizers that support it. Only used if the chosen optimizer accepts a momentum
            parameter.
        foreach: Enable/disable foreach (multi-tensor) implementation if available. If None, will use optimizer-specific
            defaults.
        filter_bias_and_bn: If True, bias, norm layer parameters (all 1d params) will not have weight decay applied.
            Only used when model_or_params is a model and weight_decay > 0.
        fallback_list: Collection of parameter name patterns to use fallback optimizer for hybrid optimizers (e.g.,
            AdamW for Muon). Supports wildcard matching.
        fallback_no_weight_decay: If True, params in model's no_weight_decay() list will use fallback optimizer for
            hybrid optimizers (e.g., AdamW for Muon).
        layer_decay: Optional layer-wise learning rate decay factor. If provided, learning rates will be scaled by
            layer_decay^(max_depth - layer_depth). Only used when model_or_params is a model.
        param_group_fn: Optional function to create custom parameter groups. If provided, other parameter grouping
    ...
    Examples:
        ...
        ...     ]
        >>> optimizer = create_optimizer_v2(model, "sgd", param_group_fn=group_fn)
