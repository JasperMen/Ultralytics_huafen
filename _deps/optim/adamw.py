...
NOTE: This impl has been deprecated in favor of torch.optim.AdamW and remains as a reference
"""
...
class AdamWLegacy(Optimizer):
    ...
    Args:
        ...
        foreach: whether foreach implementation of optimizer is used. If unspecified by the user (so foreach is None),
            we will try to use foreach over for-loop implementation on CUDA, since it is faster in general.
        capturable: whether this instance is safe to capture in a CUDA graph. Passing True can impair ungraphed
            performance, so if you don't intend to graph capture this instance, leave it False
...
def adamw(
    ...
    *,
    ...
    r"""Functional API that performs AdamW algorithm computation. See AdamWLegacy class for details.
    """
...
def _multi_tensor_adamw(
    ...
    *,
    ...
    if capturable:
        ...
        torch._foreach_div_(denom_base, torch._foreach_mul(bias_correction2_sqrt, step_size))
