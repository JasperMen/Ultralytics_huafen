...
class TarState:
    def __init__(self, tf: tarfile.TarFile | None = None, ti: tarfile.TarInfo | None = None):
...
def _extract_tarinfo(tf: tarfile.TarFile, parent_info: dict, extensions: set[str]):
    ...
    for i, ti in enumerate(tf):
        if not ti.isfile():
            continue
        _dirname, basename = os.path.split(ti.path)
...
def extract_tarinfos(
    root,
    class_name_to_idx: dict | None = None,
    cache_tarinfo: bool | None = None,
    extensions: list | tuple | set | None = None,
    sort: bool = True,
):
    ...
    if cache_tarinfo is None:
        cache_tarinfo = tar_bytes > 10 * 1024**3  # FIXME magic number, 10GB
    if cache_tarinfo:
...
class ReaderImageInTar(Reader):
    ...
    def __init__(self, root, class_map="", cache_tarfiles=True, cache_tarinfo=None):
        ...
        self.samples, self.targets, self.class_name_to_idx, tarfiles = extract_tarinfos(
            self.root, class_name_to_idx=class_name_to_idx, cache_tarinfo=cache_tarinfo
        )
