...
def _load_info(root, names=("_info.json", "info.json")):
    ...
    _logger.warning(
        f"Dataset info file not found at {tried}. Error: {err_str}. Falling back to provided split and size arg."
    )
    return {}
...
def log_and_continue(exn):
    ...
    _logger.warning(f"Handling webdataset error ({exn!r}). Ignoring.")
...
def _decode(sample, image_key="jpg", image_mode="RGB", target_key="cls", alt_label=""):
    """Custom sample decode * decode and convert PIL Image * cls byte string label to int * pass through JSON byte
    string (if it exists) without parse.
    """
    ...
    decoded = {"jpg": img, "cls": class_label, "json": sample.get("json", None)}
    return decoded
...
def pytorch_worker_seed():
    ...
    if worker_info is not None:
        # favor the seed already created for pytorch dataloader workers if it exists
...
class ReaderWds(Reader):
    def __init__(
        ...
        sample_shuffle_size: int | None = None,
        sample_initial_size: int | None = None,
    ):
        super().__init__()
        if wds is None:
            raise RuntimeError(
        ...
        if is_training and not self.num_samples:
            raise RuntimeError("Invalid split definition, num_samples not specified in train mode.")
