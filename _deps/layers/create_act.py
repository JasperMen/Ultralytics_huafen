...
def get_act_fn(name: LayerType | None = "relu"):
    """Activation Function Factory Fetching activation fns by name with this function allows export or torch script
    friendly functions to be returned dynamically based on current config.
    """
    if not name:
        return None
...
def get_act_layer(name: LayerType | None = "relu"):
    """Activation Layer Factory Fetching activation layers by name with this function allows export or torch script
    friendly functions to be returned dynamically based on current config.
    """
    if name is None:
        return None
    ...
    if not name:
        return None
    name = name.lower()
    if not (is_exportable() or is_scriptable()) and name in _ACT_LAYER_ME:
        return _ACT_LAYER_ME[name]
    return _ACT_LAYER_DEFAULT[name]


def create_act_layer(name: LayerType | None, inplace: bool | None = None, **kwargs):
