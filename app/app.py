import os, sys, time, cv2, json, mimetypes, socket
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'))
import numpy as np
from pathlib import Path
from datetime import datetime
from flask import Flask, request, jsonify, send_file, render_template, Response, abort
        ...
        try:
            ...
            if self.model_type == 'yolov8':
                ...
                for box in boxes:
                    ...
                    # 扩展 class_counts：{ name: (class_id, count, avg_conf_sum, total_conf) }
                    if class_name not in class_counts:
                        class_counts[class_name] = [cls_id, 1, conf]
                    else:
                        class_counts[class_name][1] += 1
                        class_counts[class_name][2] += conf

                # 计算各类别平均置信度，转换为 db.py 所需格式
                # { class_name: (class_id, count, avg_confidence) }
                class_counts_db = {
                    name: (v[0], v[1], round(v[2] / v[1], 4))
                    for name, v in class_counts.items()
                }
                # 保留旧格式供前端使用
                class_counts_simple = { name: v[1] for name, v in class_counts.items() }
                ...
                return {
                    ...
                    'class_counts': class_counts_db,
                    'stats': {
                        'total_objects': len(detections),
                        'num_classes': len(class_counts),
                        'class_counts': class_counts_simple,
    ...
    def detect_video(self, video_path, conf_threshold=0.5, iou_threshold=0.45):
        ...
        try:
            ...
            if out is None:
                ...
                return {
                    'success': False,
                    'message': '无法创建输出视频，请检查编码器支持'
                }
            
            # 处理每一帧
            frame_count = 0
            all_detections = []
            class_counts_acc = {}  # { name: [cls_id, count, conf_sum] }
            ...
            while True:
                ...
                if self.model_type == 'yolov8':
                    ...
                    for box in results.boxes:
                        ...
                        if class_name not in class_counts_acc:
                            class_counts_acc[class_name] = [cls_id, 1, conf]
                        else:
                            class_counts_acc[class_name][1] += 1
                            class_counts_acc[class_name][2] += conf
            ...
            # 计算统计信息
            process_time = int((time.time() - start_time) * 1000)
            # 转换为 db.py 格式：{ name: (cls_id, count, avg_conf) }
            class_counts_db = {
                name: (v[0], v[1], round(v[2] / v[1], 4))
                for name, v in class_counts_acc.items()
            }
            class_counts_simple = { name: v[1] for name, v in class_counts_acc.items() }
            ...
            return {
                ...
                'stats': {
                    ...
                    'num_classes': len(class_counts_acc),
                    'class_counts': class_counts_simple,
...
def detect():
    ...
    session_id = db.create_session(
        ...
        model_name     = os.path.splitext(os.path.basename(model_info.get('model_path', 'unknown')))[0],
    ...
    if result.get('success'):
        ...
        class_counts = result.get('class_counts', {})   # { class_name: (class_id, count, avg_conf) }
        db.record_class_counts(session_id, class_counts, frame_index=0)
        db.upsert_daily_stats(file_type, class_counts, duration_ms)
        db.complete_session(
            session_id,
            total_object_count  = stats.get('total_objects', 0),
            unique_class_count  = stats.get('num_classes', 0),
            processed_frames    = 1,
            output_path         = result.get('output_path', ''),
            duration_ms         = duration_ms,
        )
    else:
        db.update_session(session_id, status='failed', error_message=result.get('message', ''))
...
def stats_overview():
    """全局概览：总数、今日、趋势。"""
    return jsonify(db.query_overview())

@app.route('/api/stats/classes')
def stats_classes():
    """各类别累计检测计数。"""
    return jsonify(db.query_class_stats())
...
def stats_sessions():
    ...
    rows, total = db.query_sessions(limit, offset, stype, status)
    result = [dict(r) for r in rows]
    result.append({'__total': total})
    return jsonify(result)

@app.route('/api/stats/export')
def stats_export():
    """导出原始数据（CSV/JSON 格式可选）。"""
    date_from  = request.args.get('date_from', '')
    date_to    = request.args.get('date_to',   '')
    filter_cls = request.args.get('class',      None)
    return jsonify(db.query_export(date_from, date_to, filter_cls))
...
def handle_unexpected_error(error):
    ...
    return jsonify({
        'success': False,
        'message': f'服务端异常: {str(error)}'
    }), 500


if __name__ == '__main__':
    db.init_db()  # 启动时初始化数据库（幂等）
