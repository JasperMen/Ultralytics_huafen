...
def create_session(session_type, source_filename, source_path,
                   model_name, model_path, confidence, iou_threshold) -> int:
    """新建一条会话记录，返回 session_id。"""
    conn = get_conn()
    cur = conn.cursor()
    cur.execute('''INSERT INTO detection_sessions
        (session_type, source_filename, source_path, model_name, model_path,
         confidence, iou_threshold, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')''',
        (session_type, source_filename, source_path,
         model_name, model_path, confidence, iou_threshold))
    sid = cur.lastrowid
    conn.commit()
    conn.close()
    return sid
...
def complete_session(session_id, total_object_count, unique_class_count,
                    processed_frames, output_path, duration_ms):
    """标记会话为 completed。"""
    conn = get_conn()
    cur = conn.cursor()
    cur.execute('''UPDATE detection_sessions
        ...
        WHERE id=?''',
        (total_object_count, unique_class_count,
         processed_frames, output_path, duration_ms, session_id))
    conn.commit()
    conn.close()
...
def record_class_counts(session_id, class_counts: dict, frame_index=0):
    ...
    for cls_name, (cls_id, cnt, avg_conf) in class_counts.items():
        cur.execute('''INSERT INTO detection_classes
...
def upsert_daily_stats(session_type, class_counts: dict, duration_ms):
    """
    日期级 UPSERT：
    INSERT OR REPLACE + COALESCE 增量累加。
    """
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(f'''INSERT INTO daily_stats
        ...
        ON CONFLICT(stat_date) DO UPDATE SET
            {", ".join(set_parts)}''',
        [today, total_objs, img_delta, vid_delta]
        + [class_counts.get(cls, (0, 0))[1] for cls in col_map]
        + bindings)
    conn.commit()
    conn.close()
