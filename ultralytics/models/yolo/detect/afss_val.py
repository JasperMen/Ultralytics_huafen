...
from ultralytics.afss.adapters import DetectAdapter
...
def aggregate_image_metrics(num_gt: int, num_pred: int, matched_gt: int, matched_pred: int) -> dict[str, dict[str, float]]:     
    """Convert image-level detection counts into precision/recall metrics for AFSS."""
    recall = 1.0 if num_gt == 0 else matched_gt / num_gt
    if num_pred == 0:
        precision = 1.0 if num_gt == 0 else 0.0
    else:
        precision = matched_pred / num_pred 
    return {"box": {"precision": precision, "recall": recall}}   


class AFSSDetectionEvaluator(DetectionValidator):
    """Detection validator variant that keeps per-image AFSS sufficiency payloads."""    
 
    def __init__(self, dataloader=None, save_dir=None, args=None, _callbacks=None) -> None:
        super().__init__(dataloader=dataloader, save_dir=save_dir, args=args, _callbacks=_callbacks)    
        self.afss = AFSSBaseEvaluator(DetectAdapter())
        self.image_results: dict[str, dict[str, object]] = {}

    def init_metrics(self, model) -> None:
        """Initialize normal detection validator state and reset AFSS image payloads."""
    ...
    def build_image_result(self, metrics: dict[str, dict[str, float]]) -> dict[str, object]:
        """Build the AFSS payload shape expected by the detect adapter and trainer."""     
    ...
    def finalize_metrics(self) -> None:
        """Keep speed metadata available for debugging without computing detection mAP summaries."""
        self.metrics.speed = self.speed
 
    def gather_stats(self) -> None:
        """AFSS detect evaluation is single-process for the current rollout."""
        return   
