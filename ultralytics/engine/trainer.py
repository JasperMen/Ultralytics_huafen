...
class BaseTrainer:     
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):     
        ...
        self.fitness = None
        self.fitness_prev_best = None  
        self.fitness_delta = None
        self.fitness_improved = False
        self.loss = None 
        self.tloss = None   
    ...
    def _do_train(self):
        ...
        while True:   
            ...
            current_nb = len(self.train_loader)
            ...
            if RANK in {-1, 0}:
                ...
                pbar = TQDM(enumerate(self.train_loader), total=current_nb) 
            self.tloss = None
            for i, batch in pbar:
                ...
                with autocast(self.amp):
                    ...
                    # -----------------------------------------------------------
                    # Add check for NaN loss: Terminate immediately if detected  
                    # -----------------------------------------------------------   
                    if torch.isnan(self.loss):   
                        LOGGER.error(f"{BOLD}{RED}Loss is NaN at epoch {epoch}, iter {i}. Terminating training.{RESET}")
                        LOGGER.error(f"{BOLD}{RED}请尝试把AMP参数设置False以关闭混合精度训练再进行尝试，还是NAN请更换下一个模块。{RESET}")
                        raise RuntimeError(f"Training terminated: Loss is NaN at epoch {epoch}, batch {i}.")   
                    # -----------------------------------------------------------
    ...
    def save_model(self):   
        ...
        torch.save(
            {   
                ...
                "ema": deepcopy(unwrap_model(self.ema.ema)), # 不转换fp16，避免一些在fp16上精度差的模块出问题
                "updates": self.ema.updates,  
                # "optimizer": convert_optimizer_state_dict_to_fp16(deepcopy(self.optimizer.state_dict())), 
                "optimizer": deepcopy(self.optimizer.state_dict()), # 不转换fp16，避免一些在fp16上精度差的模块出问题 
    ...
    def validate(self):    
        ...
        self.fitness_prev_best = self.best_fitness
        self.fitness_delta = 0.0 if self.fitness_prev_best is None else fitness - self.fitness_prev_best    
        self.fitness_improved = not self.best_fitness or self.best_fitness < fitness
        if self.fitness_improved:   
            self.best_fitness = fitness   
        return metrics, fitness

    def _log_fitness_progress(self):
        """Log fitness trend for the current validation epoch.""" 
        if self.fitness is None or self.fitness_delta is None:     
            return
        current = float(self.fitness)
        prev_best = "N/A" if self.fitness_prev_best is None else f"{float(self.fitness_prev_best):.5f}"
        delta = float(self.fitness_delta)    
        best = "N/A" if self.best_fitness is None else f"{float(self.best_fitness):.5f}" 
        updated = " [best updated]" if self.fitness_improved else ""
        LOGGER.info(f"{BOLD}{ORANGE}Fitness: current={current:.5f}, prev_best={prev_best}, delta={delta:+.5f}, best={best}{updated}{RESET}") 
        ...
        for module_name, module in unwrap_model(model).named_modules():
            for param_name, param in module.named_parameters(recurse=False):     
                ...
                if use_muon and param.ndim >= 2:
                    # 计算除第0维以外的所有维度乘积 (Fan-in)
                    # 例如 [256, 1, 1] -> fan_in = 1 (不适合)     
                    # 例如 [64, 3, 3, 3] -> fan_in = 27 (适合) 
                    fan_in = 1
                    for d in param.shape[1:]:  
                        fan_in *= d
  
                    # 只有当输出维度(dim0) > 1 且 输入维度(fan_in) > 1 时，才视为矩阵进行 Muon 优化  
                    if param.size(0) > 1 and fan_in > 1:    
                        is_muon_candidate = True    
                    else:
                        # print(fullname)
                        pass     

                if is_muon_candidate and use_muon: 
