...
class Results(SimpleClass, DataExportMixin):
    ...
    def plot( 
        ...
        if pred_masks and show_masks:   
            ...
            # Choose mask color index source:
            # - `instance`: use tracking id when available; fallback to detection order
            # - `class`: use class id so same class keeps same color     
            idx = (
                pred_boxes.id 
            ...
            # `colors(x, True)` returns BGR tuples from the global plotting palette.    
        ...
        if pred_boxes is not None and show_boxes:
            for i, d in enumerate(reversed(pred_boxes)):
                ...
                # Box color follows `color_mode`:   
                # class mode -> class id; instance mode -> track id (or loop index if no id).
                annotator.box_label(    
                    box,
                    label,
                    color=colors(
