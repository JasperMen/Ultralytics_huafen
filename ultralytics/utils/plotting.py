...
def feature_visualization(x, module_type: str, stage: int, n: int = 32, save_dir: Path = Path("runs/detect/exp")):  
    ...
    Saves per-channel maps plus one channel-sum map for each sample.
  
    Args:
        x (torch.Tensor): Features to be visualized.
        module_type (str): Module type.
        stage (int): Module stage within the model.   
        n (int, optional): Reserved for backward compatibility. All channels are saved when visualize is enabled.  
    ...
    if isinstance(x, torch.Tensor):   
        ...
        if height > 1 and width > 1:   
            module_name = module_type.rsplit(".", 1)[-1]
            stage_dir = save_dir / f"stage{stage}_{module_name}"    
            stage_dir.mkdir(parents=True, exist_ok=True)     
            x_cpu = x.detach().cpu().numpy()
            n = channels  # visualize=True saves all channel feature maps.
    
            for b in range(batch):     
                sample_dir = stage_dir / f"sample_{b:03d}"
                sample_dir.mkdir(parents=True, exist_ok=True)
                np.save(str(sample_dir / "features.npy"), x_cpu[b])  # [C, H, W]
                sum_map = x_cpu[b].sum(axis=0)  # [H, W], direct channel-wise sum     
                plt.imsave(str(sample_dir / "channels_sum.png"), sum_map, cmap="viridis")
                for i in range(n):   
                    plt.imsave(str(sample_dir / f"ch_{i:04d}.png"), x_cpu[b, i], cmap="viridis")

            LOGGER.info(    
                f"Saving {batch * channels} feature maps and {batch} channel-sum maps to {stage_dir}..."
                f" ({batch} samples x {channels} channels)"
            ) 
