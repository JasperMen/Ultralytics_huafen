...
def model_info(model, detailed=False, verbose=True, imgsz=640): 
    ...
    LOGGER.info(
        f"{BOLD}{CYAN}{model_name}{RESET} summary{CYAN}{fused}{RESET}: "    
        f"{BOLD}{GREEN}{n_l:,}{RESET} layers, "  
        f"{BOLD}{GREEN}{n_p:,}{RESET} parameters, " 
        f"{BOLD}{GREEN}{n_g:,}{RESET} gradients"
        f"{BOLD}{YELLOW}{fs}{RESET}"     
    )  
...
def strip_optimizer(f: str | Path = "best.pt", s: str = "", updates: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if s:
        fp32_path = s
        # Generate FP16 path by adding suffix before extension
        s_path = Path(s)
        fp16_path = s_path.parent / f"{s_path.stem}_fp16{s_path.suffix}"
    else:
        f_path = Path(f)   
        fp32_path = f_path.parent / f"{f_path.stem}_fp32{f_path.suffix}"
        fp16_path = f_path.parent / f"{f_path.stem}_fp16{f_path.suffix}"
    
    # Save FP32 model  
    torch.save(combined_fp32, fp32_path)
    mb_fp32 = os.path.getsize(fp32_path) / 1e6    
    LOGGER.info(f"FP32 model saved to {fp32_path}, {mb_fp32:.1f}MB")     
    
    # Create FP16 version
    x["model"].half()  # convert to FP16
    combined_fp16 = {**metadata, **x, **(updates or {})}  
    
    # Save FP16 model
    torch.save(combined_fp16, fp16_path)  
    mb_fp16 = os.path.getsize(fp16_path) / 1e6     
    LOGGER.info(f"FP16 model saved to {fp16_path}, {mb_fp16:.1f}MB")   
    LOGGER.info(f"Optimizer stripped from {f}, saved as FP32 and FP16 versions")
   
    return combined_fp32     
