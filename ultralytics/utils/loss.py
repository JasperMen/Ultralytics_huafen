...
def _validate_detection_loss_args(args: Any) -> None:
    ...
    if _parse_iou_loss_mode(iou_loss) is None:
        raise ValueError(
            f"Unsupported iou_loss='{iou_loss}'. Supported forms: base variants {sorted(BASE_IOU_VARIANTS)}, "
            f"`inner_<variant>`, `focaler_<variant>`, {sorted(MPD_IOU_VARIANTS)}, "
            "`wiseiou`, `wiseiou_<variant>`, `wiseiou_inner_<variant>`, `wiseiou_focaler_<variant>`; "
            f"wise variants are {sorted(WISE_IOU_VARIANTS)}."
        )
...
class QualityfocalLoss_YOLO(nn.Module):
    ...
    def forward(
        self, pred_score: torch.Tensor, gt_score: torch.Tensor, gt_target_pos_mask: torch.Tensor
    ) -> torch.Tensor:
...
class BboxLoss(nn.Module):
    ...
    def _iou_term(self, pred: torch.Tensor, target: torch.Tensor, mpdiou_hw: torch.Tensor) -> torch.Tensor:
        ...
        if family == "mpd":
            ...
            return bbox_focaler_mpdiou(
                pred, target, xywh=False, mpdiou_hw=mpdiou_hw, d=self.focaler_d, u=self.focaler_u
            )
...
class v8DetectionLoss:
    ...
    def _format_selected_losses(self) -> str:
        ...
        if (
            mode["family"] == "inner"
            or (mode["family"] == "mpd" and mode["variant"] == "inner_mpdiou")
            or (mode["family"] == "wise" and mode.get("inner", False))
        ):
            extras.append(f"inner_iou_ratio={float(_get_arg(self.hyp, 'inner_iou_ratio', 0.7)):.3f}")
        if (
            mode["family"] == "focaler"
            or (mode["family"] == "mpd" and mode["variant"] == "focaler_mpdiou")
            or (mode["family"] == "wise" and mode.get("focaler", False))
        ):
