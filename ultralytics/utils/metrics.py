...
def bbox_iou(     
    ...
    Args:
        ...
        ShapeIoU (bool, optional): If True, calculate ShapeIoU.
        PIoU (bool, optional): If True, calculate PIoU.
        PIoU2 (bool, optional): If True, calculate PIoU2.  
        eps (float, optional): A small value to avoid division by zero.
 
    Returns:
        (torch.Tensor): IoU variant values depending on the specified flags.
    """ 
    # Get the coordinates of bounding boxes 
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or PIoU or PIoU2:
        ...
        if CIoU or DIoU or EIoU or SIoU or PIoU or PIoU2 or ShapeIoU:  # Distance or Complete IoU    
...
def get_inner_iou(box1: torch.Tensor, box2: torch.Tensor, xywh: bool = True, eps: float = 1e-7, ratio: float = 0.7):  
    """Compute Inner-IoU used by Inner-IoU variants."""    
    
    def xyxy2xywh_single(x):
    ...
    if not xywh:
        box1, box2 = xyxy2xywh_single(box1), xyxy2xywh_single(box2)  
    (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
...
def bbox_inner_iou(
    ...
    """Compute IoU variants using Inner-IoU as the base overlap term."""
    if xywh:
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
        w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
        b1_x1, b1_x2, b1_y1, b1_y2 = x1 - w1_, x1 + w1_, y1 - h1_, y1 + h1_     
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or PIoU or PIoU2:  
        ...
        if CIoU or DIoU or EIoU or SIoU or PIoU or PIoU2 or ShapeIoU:     
            ...
            if CIoU:
                ...
                cw2 = cw**2 + eps   
                ch2 = ch**2 + eps
                return inner_iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2) 
            elif SIoU:  
                s_cw = (b2_x1 + b2_x2 - b1_x1 - b1_x2) * 0.5 + eps     
                s_ch = (b2_y1 + b2_y2 - b1_y1 - b1_y2) * 0.5 + eps
                sigma = torch.pow(s_cw**2 + s_ch**2, 0.5)     
                ...
                omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
                shape_cost = torch.pow(1 - torch.exp(-omiga_w), 4) + torch.pow(1 - torch.exp(-omiga_h), 4)   
                return inner_iou - 0.5 * (distance_cost + shape_cost) + eps
            elif ShapeIoU:   
                ww = 2 * torch.pow(w2, scale) / (torch.pow(w2, scale) + torch.pow(h2, scale))
                hh = 2 * torch.pow(h2, scale) / (torch.pow(w2, scale) + torch.pow(h2, scale))  
                cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)
                ...
                omiga_h = ww * torch.abs(h1 - h2) / torch.max(h1, h2)
                shape_cost = torch.pow(1 - torch.exp(-omiga_w), 4) + torch.pow(1 - torch.exp(-omiga_h), 4)
                return inner_iou - distance - 0.5 * shape_cost  
            elif PIoU or PIoU2:   
                dw1 = torch.abs(b1_x2.minimum(b1_x1) - b2_x2.minimum(b2_x1))  
                dw2 = torch.abs(b1_x2.maximum(b1_x1) - b2_x2.maximum(b2_x1))
                dh1 = torch.abs(b1_y2.minimum(b1_y1) - b2_y2.minimum(b2_y1))
                dh2 = torch.abs(b1_y2.maximum(b1_y1) - b2_y2.maximum(b2_y1))  
                P = ((dw1 + dw2) / (torch.abs(w2) + eps) + (dh1 + dh2) / (torch.abs(h2) + eps)) / 4
                piou_v1 = 1 - inner_iou - torch.exp(-P**2) + 1     
                if PIoU:
                    return 1 - piou_v1
                elif PIoU2:     
                    q = torch.exp(-P)    
                    x = q * Lambda
                    return 1 - 3 * x * torch.exp(-(x**2)) * piou_v1
            return inner_iou - rho2 / c2    
        c_area = cw * ch + eps   
        return inner_iou - (c_area - union) / c_area
    return inner_iou

   
def bbox_focaler_iou(     
    box1: torch.Tensor,     
    box2: torch.Tensor,
    xywh: bool = True,
    GIoU: bool = False,  
    ...
    """Compute IoU variants using Focaler-IoU as base overlap term."""     
    iou = bbox_iou(     
        box1=box1,   
        box2=box2,
        xywh=xywh,
...
def bbox_mpdiou(box1: torch.Tensor, box2: torch.Tensor, xywh: bool = True, mpdiou_hw=1, eps: float = 1e-7):
    """Compute MPDIoU."""     
    if xywh:    
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)     
        w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
    ...
    mpdiou_hw = torch.as_tensor(mpdiou_hw, device=box1.device, dtype=box1.dtype) 
    return iou - d1 / (mpdiou_hw.unsqueeze(1) + eps) - d2 / (mpdiou_hw.unsqueeze(1) + eps)     
...
def bbox_inner_mpdiou( 
    ...
    d2 = (b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2
    mpdiou_hw = torch.as_tensor(mpdiou_hw, device=box1.device, dtype=box1.dtype) 
    return inner_iou - d1 / (mpdiou_hw.unsqueeze(1) + eps) - d2 / (mpdiou_hw.unsqueeze(1) + eps)
 
    
def bbox_focaler_mpdiou(
    box1: torch.Tensor,
    box2: torch.Tensor,    
    xywh: bool = True,
    mpdiou_hw=1,
...
def wasserstein_loss(pred: torch.Tensor, target: torch.Tensor, eps: float = 1e-7, constant: float = 12.8):
    """Compute normalized Wasserstein distance based overlap."""    
...
def gcd_loss(pred: torch.Tensor, target: torch.Tensor, eps: float = 1e-7, mode: str = "exp", gamma: float = 1): 
    """Compute generalized center-distance overlap.""" 
    ...
    if mode == "exp": 
        return torch.exp(-torch.sqrt(gcd_2))  
    if mode == "sqrt":     
        return torch.sqrt(gcd_2)
    if mode == "log":     
        return torch.log(gcd_2 + 1)     
    if mode == "norm_sqrt":
        return 1 - 1 / (gamma + torch.sqrt(gcd_2))
    if mode == "w2":     
        return gcd_2
    raise ValueError(f"Unsupported gcd mode: {mode}")   
...
class WiseIouLoss(torch.nn.Module):  
    ...
    def __init__(self, ltype: str = "WIoU", monotonous: bool | None = False, inner_iou: bool = False, focaler_iou: bool = False):
        super().__init__()    
    ...
    def forward(self, pred, target, ret_iou: bool = False, ratio: float = 1.0, d: float = 0.0, u: float = 0.95, **kwargs):
        self._fget = {
            "pred": pred,     
            "target": target,
            ...
            "s_union": lambda: torch.prod(self["pred_wh"], dim=-1) + torch.prod(self["target_wh"], dim=-1) - self["s_inter"],     
            ...
            "iou": lambda: (    
                1 - get_inner_iou(pred, target, xywh=False, ratio=ratio).squeeze()
                if self.inner_iou
                else (
                    1 - ((self["s_inter"] / self["s_union"] - d) / (u - d)).clamp(0, 1)
                    if self.focaler_iou     
                    else 1 - self["s_inter"] / self["s_union"]
                )  
            ),
        }    
        if self.training:    
    ...
    def _CIoU(self, eps: float = 1e-4):
        v = (4 / math.pi**2) * (   
            torch.atan(self["pred_wh"][..., 0] / (self["pred_wh"][..., 1] + eps)) 
            - torch.atan(self["target_wh"][..., 0] / (self["target_wh"][..., 1] + eps))
        ) ** 2  
        alpha = v / (self["iou"] + v)
        return self["iou"] + self["l2_center"] / self["l2_box"] + alpha.detach() * v
   
    def _SIoU(self, theta: float = 4):
    ...
    def _ShapeIoU(self, scale: float = 0.0):
    ...
    def _PIoU2(self, Lambda: float = 1.3):
        b1_x1, b1_y1, b1_x2, b1_y2 = self["pred"].chunk(4, -1) 
        b2_x1, b2_y1, b2_x2, b2_y2 = self["target"].chunk(4, -1)   
        w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1 + 1e-7
        dw1 = torch.abs(b1_x2.minimum(b1_x1) - b2_x2.minimum(b2_x1))  
...
class Metric(SimpleClass):   
    ...
    def mean_results_with_map75(self) -> list[float]:
        """Return mean of results, mp, mr, map50, map75, map."""
        return [self.mp, self.mr, self.map50, self.map75, self.map]

    def class_result_with_map75(self, i: int) -> tuple[float, float, float, float, float]:   
        """Return class-aware result with AP75, p[i], r[i], ap50[i], ap75[i], ap[i]."""
    ...
    def fitness(self) -> float:  
        ...
        w = [0.0, 0.0, 1.0, 1.0]  # weights for [P, R, mAP@0.5, mAP@0.5:0.95]    
...
class DetMetrics(SimpleClass, DataExportMixin):
    ...
    def mean_results_with_map75(self) -> list[float]: 
        ...
        return self.box.mean_results_with_map75()
     
    def class_result_with_map75(self, i: int) -> tuple[float, float, float, float, float]:
        """Return per-class detection result with AP75."""
        return self.box.class_result_with_map75(i)
...
class SegmentMetrics(DetMetrics):
    ...
    def mean_results_with_map75(self) -> list[float]:
        """Return the mean metrics for bounding box and segmentation results with mAP75 included."""
        return DetMetrics.mean_results_with_map75(self) + self.seg.mean_results_with_map75()

    def class_result_with_map75(self, i: int) -> list[float]:    
        """Return classification results for a specified class index with AP75 included."""  
        return DetMetrics.class_result_with_map75(self, i) + self.seg.class_result_with_map75(i)
...
class PoseMetrics(DetMetrics):  
    ...
    def mean_results_with_map75(self) -> list[float]:
        """Return the mean results of box and pose with mAP75 included."""
        return DetMetrics.mean_results_with_map75(self) + self.pose.mean_results_with_map75()   

    def class_result_with_map75(self, i: int) -> list[float]:
        """Return the class-wise detection results for a specific class i with AP75 included."""
        return DetMetrics.class_result_with_map75(self, i) + self.pose.class_result_with_map75(i) 
