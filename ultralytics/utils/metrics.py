...
class WiseIouLoss(torch.nn.Module):
    ...
    def __init__(
        self, ltype: str = "WIoU", monotonous: bool | None = False, inner_iou: bool = False, focaler_iou: bool = False
    ):
        super().__init__()
    ...
    def forward(
        self, pred, target, ret_iou: bool = False, ratio: float = 1.0, d: float = 0.0, u: float = 0.95, **kwargs
    ):
        self._fget = {
            "pred": pred,
            "target": target,
            ...
            "s_union": lambda: (
                torch.prod(self["pred_wh"], dim=-1) + torch.prod(self["target_wh"], dim=-1) - self["s_inter"]
            ),
