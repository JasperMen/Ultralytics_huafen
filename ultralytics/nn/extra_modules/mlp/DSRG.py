"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/自研模块-DSRG.md
ultralytics/nn/module_images/自研模块-DSRG.png.
"""
...
class DSRG(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU):
        ...
        self.linear1 = nn.Sequential(nn.Conv2d(in_features, hidden_features, 1), act_layer())
        self.local_dwconv = nn.Sequential(
            nn.Conv2d(self.hidden_dim, self.hidden_dim, 3, 1, 1, groups=self.hidden_dim, bias=False), act_layer()
        )
    ...
    def forward(self, x):
        ...
        feat = local * context_weight[:, 0].view(-1, 1, 1, 1) + context * context_weight[:, 1].view(-1, 1, 1, 1)
