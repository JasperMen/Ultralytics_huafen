"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2024-CGLU.png
...
class ConvolutionalGLU(nn.Module):
    ...
    def forward(self, x):
        """前向传播过程： 1. 先存储输入 x 作为残差连接的 shortcut。 2. 通过 1x1 卷积 self.fc1，将输入通道扩展为 2 倍，并分成两个部分 (x, v)。 3. x 经过深度可分离卷积
        self.dwconv 处理后，与门控信号 v 相乘，实现门控机制。 4. 经过 Dropout 防止过拟合。 5. 通过 1x1 卷积 self.fc2 将通道数恢复到输出通道数。 6. 再次进行
        Dropout。 7. 残差连接，将原始输入 x_shortcut 与处理后的 x 相加。.
        """
        # 残差连接的快捷分支
...
if __name__ == "__main__":
    ...
    module = ConvolutionalGLU(in_features=in_channel, hidden_features=hidden_channel, out_features=out_channel).to(
        device
    )
