"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2023-FasterBlock.png
...
class Faster_Block(nn.Module):
    def __init__(
        self, inc, ouc, n_div=4, mlp_ratio=2, drop_path=0.1, layer_scale_init_value=0.0, pconv_fw_type="split_cat"
    ):
        super().__init__()
        self.ouc = ouc
        ...
        mlp_layer = [Conv(ouc, mlp_hidden_dim, 1), nn.Conv2d(mlp_hidden_dim, ouc, 1, bias=False)]

        self.mlp = nn.Sequential(*mlp_layer)

        self.spatial_mixing = Partial_conv3(ouc, n_div, pconv_fw_type)
...
class Faster_Block_CGLU(Faster_Block):
    def __init__(
        self, inc, ouc, n_div=4, mlp_ratio=2, drop_path=0.1, layer_scale_init_value=0, pconv_fw_type="split_cat"
    ):
