"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/TIP2026-FourierSR.png
ultralytics/nn/module_images/TIP2026-FourierSR.md
...
class FourierSR(nn.Module):
    """channels: channel dimension size num_blocks: how many blocks to use in the block diagonal weight matrices (higher
    => less complexity but less parameters) sparsity_threshold: lambda for softshrink hard_thresholding_fraction:
    how many frequencies you want to completely mask out (lower => hard_thresholding_fraction^2 less FLOPs) input
    shape [B N C].
    """

    def __init__(self, in_channels, out_channels, num_blocks=8, sparsity_threshold=0.01):
        super().__init__()
        assert in_channels % num_blocks == 0, f"in_channels {in_channels} should be divisible by num_blocks {num_blocks}"
