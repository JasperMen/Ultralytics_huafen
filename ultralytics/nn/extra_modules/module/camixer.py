"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2024-CAMixer.md
...
def flow_warp(x, flow, interp_mode="bilinear", padding_mode="zeros", align_corners=True):
    ...
    Args:
        ...
        Default: 'zeros'.
        align_corners (bool): Before pytorch 1.3, the default value is align_corners=True. After pytorch 1.3, the
            default value is align_corners=False. Here, we use the True as default.
    ...
    grid = torch.stack((grid_x, grid_y), 2).to(dtype=x.dtype)  # W(x), H(y), 2
...
class CAMixer(nn.Module):
    def __init__(self, in_dim, dim, window_size=4, bias=True, is_deformable=True, ratio=0.5):
        super().__init__()
    ...
    def forward(self, x, condition_global=None, mask=None, train_mode=False):
        ...
        f_attn = rearrange(
            f_attn, "(b n) (dh dw) c -> b n (dh dw c)", b=N, n=N_, dh=self.window_size, dw=self.window_size
        )
...
if __name__ == "__main__":
    ...
    module = CAMixer(in_channel, out_channel, window_size=8).to(device)
