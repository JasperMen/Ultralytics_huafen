...
def resize_complex_weight(origin_weight, new_h, new_w):
    ...
    new_weight = (
        torch.nn.functional.interpolate(origin_weight, size=(new_h, new_w), mode="bicubic", align_corners=True)
        .permute(0, 2, 3, 1)
        .reshape(new_h, new_w, num_heads, 2)
    )
    return new_weight
...
class DynamicFilterMlp(nn.Module):
    """MLP as used in MetaFormer models, eg Transformer, MLP-Mixer, PoolFormer, MetaFormer baselines and related
