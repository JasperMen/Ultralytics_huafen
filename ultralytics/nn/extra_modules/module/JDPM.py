    ...
    def forward(self, F1):
        ...
        F1_3_f = self.relu(
            self.norm(
                torch.abs(
                    torch.fft.ifft2(
                        self.weight(F1_3_fft.real.to(dtype=F1_3_s.dtype)).to(dtype=F1_3_fft.real.dtype) * F1_3_fft
                    )
                ).to(dtype=F1_3_s.dtype)
            )
        )
        ...
        F1_5_f = self.relu(
            self.norm(
                torch.abs(
                    torch.fft.ifft2(
                        self.weight(F1_5_fft.real.to(dtype=F1_5_s.dtype)).to(dtype=F1_5_fft.real.dtype) * F1_5_fft
                    )
                ).to(dtype=F1_5_s.dtype)
            )
        )
        ...
        F1_7_f = self.relu(
            self.norm(
                torch.abs(
                    torch.fft.ifft2(
                        self.weight(F1_7_fft.real.to(dtype=F1_7_s.dtype)).to(dtype=F1_7_fft.real.dtype) * F1_7_fft
                    )
                ).to(dtype=F1_7_s.dtype)
            )
        )
        ...
        F1_9_f = self.relu(
            self.norm(
                torch.abs(
                    torch.fft.ifft2(
                        self.weight(F1_9_fft.real.to(dtype=F1_9_s.dtype)).to(dtype=F1_9_fft.real.dtype) * F1_9_fft
                    )
                ).to(dtype=F1_9_s.dtype)
            )
        )
