"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/ICLR2024-FATBlock.png
...
def img2windows(img, H_sp, W_sp):
    """Input: Image (B, C, H, W) Output: Window Partition (B', N, C).
    """
    B, C, H, W = img.shape
...
def windows2img(img_splits_hw, H_sp, W_sp, H, W):
    """Input: Window Partition (B', N, C) Output: Image (B, H, W, C).
    """
...
class WindowAttention(nn.Module):
    ...
    def forward(self, qkv, H, W, mask=None):
        """Input: qkv: (B, 3*L, C), H, W, mask: (B, N, N), N is the window size Output: x (B, H, W, C).
        """
...
class Swin_FDWA(nn.Module):
    ...
    def calculate_mask(self, H, W, split_size=None):
        ...
        h_slices_0 = (slice(0, -split_size[0]), slice(-split_size[0], -shift_size[0]), slice(-shift_size[0], None))
        w_slices_0 = (slice(0, -split_size[1]), slice(-split_size[1], -shift_size[1]), slice(-shift_size[1], None))
        ...
        img_mask_0 = (
            img_mask_0.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, split_size[0], split_size[1], 1)
        )  # nW, sw[0], sw[1], 1
...
class WindowFrequencyModulation(nn.Module):
    ...
    def forward(self, x, H, W, spatial_size=None):
        ...
        x = x.view(B, -1, C).to(dtype=input_dtype)
        return x
...
if __name__ == "__main__":
    ...
    batch_size, in_channel, out_channel, height, width = 1, 16, 32, 40, 40
