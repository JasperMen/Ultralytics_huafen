"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2025-EfficientVIM.png
...
class EfficientViMBlock(nn.Module):
    def __init__(self, inc, ouc, mlp_ratio=4.0, ssd_expand=1, state_dim=64):
        super().__init__()
        self.dim = inc
    ...
    def forward(self, x):
        ...
        x_prev = x
        _, _, H, W = x.size()
        x, _h = self.mixer(self.norm(x.flatten(2)), (H, W))
