...
class CNCM(nn.Module):
    def __init__(self, channel_in, channel_out, reduction=16):
        ...
        # RCSSC
        self.unit_1 = CSSC(int(channel_in / 2.0), int(channel_in / 2.0), reduction)
        self.unit_2 = CSSC(int(channel_in / 2.0), int(channel_in / 2.0), reduction)
        ...
        self.conv1x1 = (
            Conv(channel_in, channel_out, 1, act=nn.LeakyReLU) if channel_in != channel_out else nn.Identity()
        )
