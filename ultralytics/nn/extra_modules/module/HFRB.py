"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/ICCV2025-HFRB.png
ultralytics/nn/module_images/ICCV2025-HFRB.md
...
class PreNorm(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        x = F.layer_norm(
            x.float(), self.norm.normalized_shape, self.norm.weight.float(), self.norm.bias.float(), self.norm.eps
        ).to(dtype=x.dtype)
...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        if torch.__version__ > "1.7.1":
            ...
            ffted = self.conv_layer(ffted.to(dtype=self.conv_layer.weight.dtype))  # (batch, c*2, h, w/2+1)
...
class SpectralTransform(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.conv2((x + output + xs).to(dtype=self.conv2.weight.dtype))
...
class HFRB(nn.Module):
    def __init__(self, dims, out_dims, d_state=16, n_l_block=1, n_h_block=1, LayerNorm_type="WithBias"):
        super().__init__()
        self.dim = dims
        ...
        self.conv1x1 = Conv(dims, out_dims, 1) if dims != out_dims else nn.Identity()
    ...
    def create_wave_conv(self):
        horizontal_kernel = (
            torch.tensor([[1, 0, -1], [1, 0, -1], [1, 0, -1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        )

        vertical_kernel = (
            torch.tensor([[1, 1, 1], [0, 0, 0], [-1, -1, -1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        )

        diagonal_kernel = (
            torch.tensor([[0, 1, 0], [1, -4, 1], [0, 1, 0]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        )
...
if __name__ == "__main__":
    ...
    module = HFRB(in_channel, out_channel).to(device)
