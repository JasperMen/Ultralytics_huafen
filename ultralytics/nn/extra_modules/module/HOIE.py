...
class HOIE(nn.Module):
    ...
    Extends IEL's second-order multiplicative interaction (x1*x2) to a three-path architecture with explicit first-,
    second-, and third-order interaction terms and order-adaptive fusion.
    ...
    def __init__(self, in_dim, out_dim, ffn_expansion_factor=2.66, bias=False):
        ...
        self.dwconv_pre = nn.Conv2d(
            total_expand, total_expand, kernel_size=3, stride=1, padding=1, groups=total_expand, bias=bias
        )

        # --- Three enhancement paths ---
        # Path 1 & 2: 3x3 DWConv (standard detail capture)
        self.dwconv1 = nn.Conv2d(self.C_h, self.C_h, kernel_size=3, stride=1, padding=1, groups=self.C_h, bias=bias)
        self.dwconv2 = nn.Conv2d(self.C_h, self.C_h, kernel_size=3, stride=1, padding=1, groups=self.C_h, bias=bias)
        # Path 3: 5x5 DWConv (extended receptive field)
        self.dwconv3 = nn.Conv2d(self.C_h, self.C_h, kernel_size=5, stride=1, padding=2, groups=self.C_h, bias=bias)
