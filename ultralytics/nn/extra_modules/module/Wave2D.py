...
class Wave2D(nn.Module):
    r"""Wave equation operator: d2u/dt2 - c2(d2u/dx2 + d2u/dy2) + αdu/dt = 0; du/dx_{x=0, x=a} = 0 du/dy_{y=0, y=b} = 0
    => A_{n, m} = C(a, b, n==0, m==0) * sum_{0}^{a}{ sum_{0}^{b}{\phi(x, y)cos(n\pi/ax)cos(m\pi/by)dxdy }} core =
    cos(n\pi/ax)cos(m\pi/by) * (1 - [(n\pi/a)^2 + (m\pi/b)^2]c2t2) * e^(-αt) u_{x, y, t} = sum_{0}^{\infinite}{
    sum_{0}^{\infinite}{ core } }.

    assume a = N, b = M; x in [0, N], y in [0, M]; n in [0, N], m in [0, M]; with some slight change => (\phi(x, y) =
    linear(dwconv(input(x, y)))) A(n, m) = DCT2D(\phi(x, y)) u(x, y, t) = IDCT2D(A(n, m) * (1 - [(n\pi/a)^2 +
    (m\pi/b)^2]c2t2) * e^(-αt))
    """

    def __init__(self, in_dim=96, out_dim=96, res=None, **kwargs):
        if res is None:
            res = [14, 14]
        super().__init__()
        self.res = res
        ...
        trunc_normal_(self.freq_embed, std=0.015)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if (
            ...
            and cached_weight_cosn is not None
            and (cached_weight_cosn.device == x.device)
        ):
            ...
            self.__RES__ = H, W
            self.__WEIGHT_COSN__ = weight_cosn
            self.__WEIGHT_COSM__ = weight_cosm
            self.__WEIGHT_EXP__ = weight_exp

        _N, _M = weight_cosn.shape[0], weight_cosm.shape[0]
        ...
        if (
            ((H, W) == getattr(self, "__RES_IDCT__", (0, 0)))
            and cached_weight_cosn_idct is not None
            and cached_weight_cosn_idct.device == x.device
        ):
            ...
            self.__RES_IDCT__ = H, W
            self.__WEIGHT_COSN_IDCT__ = weight_cosn
            self.__WEIGHT_COSM_IDCT__ = weight_cosm
