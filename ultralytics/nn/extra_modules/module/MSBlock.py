...
class MSBlock(nn.Module):
    def __init__(
        self, inc, ouc, kernel_sizes=None, in_expand_ratio=3.0, mid_expand_ratio=2.0, layers_num=3, in_down_ratio=2.0
    ) -> None:
...
class MSBlock_GQL(nn.Module):
    def __init__(
        ...
        gql_param=(3, 4),
        start_branch_id=1,
        channel_split_ratios=None,
    ) -> None:
        if channel_split_ratios is None:
            channel_split_ratios = [1 / 3, 1 / 3, 1 / 3]
        super().__init__()

        self.channel_split_ratios = [ratio / sum(channel_split_ratios) for ratio in channel_split_ratios]
        ...
        self.mid_channels = [int(ratio * self.in_channel) for ratio in self.channel_split_ratios]
        ...
        for kernel_size, group, mid_channel in zip(kernel_sizes, groups, self.mid_channels):
            if kernel_size == 1:
                self.mid_convs.append(nn.Identity())
                continue
            mid_convs = [MSBlockLayer(mid_channel, group, kernel_size) for _ in range(int(self.layers_num))]
            self.mid_convs.append(nn.Sequential(*mid_convs))
        self.mid_convs = nn.ModuleList(self.mid_convs)
