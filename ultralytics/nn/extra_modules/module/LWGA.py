"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/LWGA.png
ultralytics/nn/module_images/LWGA.md
论文链接：https://arxiv.org/pdf/2501.10040.
"""
...
class LA(nn.Module):
    def __init__(self, dim, norm_layer, act_layer):
        super().__init__()
        self.conv = nn.Sequential(nn.Conv2d(dim, dim, 3, 1, 1, bias=False), norm_layer(dim), act_layer())
...
class LWGA(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.stage = stage
        self.dim_split = indim // 4
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()

        mlp_hidden_dim = int(indim * mlp_ratio)

        mlp_layer: list[nn.Module] = [
            nn.Conv2d(indim, mlp_hidden_dim, 1, bias=False),
            norm_layer(mlp_hidden_dim),
            act_layer(),
            nn.Conv2d(mlp_hidden_dim, indim, 1, bias=False),
        ]
...
if __name__ == "__main__":
    ...
    # 对于P2/P3层 stage推荐设置为1
    # 对于P4层 stage推荐设置为2
    # 对于P5层 stage推荐设置为3
    module = LWGA(in_channel, out_channel, stage=0).to(device)
