"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2026-FrequencyCM.png
ultralytics/nn/module_images/CVPR2026-FrequencyCM.md
...
class FreMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        _, _, _H_fft, _W_fft = x_freq.shape
...
class FrequencyCM(nn.Module):
    def __init__(self, inc, ouc, DW_Expand=2, dilations=None, extra_depth_wise=True):
        ...
        self.extra_conv = (
            nn.Conv2d(inc, inc, kernel_size=3, padding=1, stride=1, groups=inc, bias=True, dilation=1)
            if extra_depth_wise
            else nn.Identity()
        )  # optional extra dw
...
if __name__ == "__main__":
    ...
    half_module = FrequencyCM(in_channel, out_channel).cuda().half().eval()
    with torch.no_grad():
        half_outputs = half_module(half_inputs)
    print(GREEN + f"half inputs.size:{half_inputs.size()} half outputs.size:{half_outputs.size()}" + RESET)
