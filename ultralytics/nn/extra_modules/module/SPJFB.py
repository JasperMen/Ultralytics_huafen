...
def compute_gradient(image):
    sobel_x = (
        torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=image.dtype, device=image.device)
        .unsqueeze(0)
        .unsqueeze(0)
    )
    sobel_y = (
        torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=image.dtype, device=image.device)
        .unsqueeze(0)
        .unsqueeze(0)
    )
...
class LowFrequencyProcessing(nn.Module):
    def __init__(self, nf=64, num_blocks=6, input_channels=3, output_channels=None):
        ...
        self.concat_layers = nn.ModuleList(
            [nn.Sequential(nn.Conv2d(nf * 2, nf, kernel_size=1, stride=1, padding=0), SEBlock(nf)) for _ in range(3)]
        )
...
class DFGFHigh(nn.Module):
    def __init__(self, in_nf=3, out_if=3, nf=16):
        ...
        self.convf1 = nn.Sequential(
            DepthConv(in_ch=nf, out_ch=nf), nn.LeakyReLU(0.1), nn.Conv2d(nf, nf, 3, 1, 1, groups=4)
        )
