...
class _FreqBandRouter(nn.Module):
    ...
    Computes a 2D FFT amplitude spectrum, splits it into low / mid / high frequency bands via radial masks, extracts
    per-band global statistics (mean amplitude), and maps them to M scale weights via a shared FC.

    The output is a (B, M) softmax weight vector — one scalar per branch. No inverse FFT is performed; the frequency
    path is purely analytical.
    """
...
class FMSI(nn.Module):
    ...
    Extends MSInit with a lightweight FFT-based frequency router that generates per-scale attention weights from the
    input's spectral
    statistics.  Each branch output is multiplied by its corresponding
    frequency-derived scalar weight before concatenation, enabling cross-domain (frequency → spatial) adaptive
