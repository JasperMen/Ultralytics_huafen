"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/ICCV2025-ESC.md
ultralytics/nn/module_images/ICCV2025-ESC.png
    ...
    def forward(self, x):
        if self.data_format == "channels_last":
            ...
            if self.training:
                return (
                    F.layer_norm(
                        x.permute(0, 2, 3, 1).contiguous(), self.normalized_shape, self.weight, self.bias, self.eps
                    )
                    .permute(0, 3, 1, 2)
                    .contiguous()
                )
...
class ConvFFN(nn.Module):
    def __init__(self, dim: int, kernel_size: int, exp_ratio: int):
        ...
        self.dwc = nn.Conv2d(
            int(dim * exp_ratio), int(dim * exp_ratio), kernel_size, 1, kernel_size // 2, groups=int(dim * exp_ratio)
        )
...
class WindowAttention(nn.Module):
    def __init__(self, dim: int, window_size: int, num_heads: int, attn_func=None, attn_type: ATTN_TYPE = "Flex"):
        super().__init__()
        self.dim = dim
...
if __name__ == "__main__":
    ...
    module = ESCBlock(in_channel, out_channel, window_size=10).to(device)
