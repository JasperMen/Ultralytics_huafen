...
class DEGConv(nn.Module):
    ...
    def get_hog_feature(self, x):
        ...
        sobel_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=torch.float32, device=device).view(
            1, 1, 3, 3
        )
        sobel_y = torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=torch.float32, device=device).view(
            1, 1, 3, 3
        )
        ...
        hog_feature = (
            torch.linspace(start, torch.pi - start, self.nbins, device=device, dtype=torch.float32).repeat(
                b, h_cells, w_cells, 1
            )
            * weight
        )
