"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2024-FADC.png
ultralytics/nn/module_images/CVPR2024-FADC.md
论文链接：https://arxiv.org/pdf/2403.05369.
"""
...
def generate_laplacian_pyramid(input_tensor, num_levels, size_align=True, mode="bilinear"):
    ...
    for _ in range(num_levels):
        _b, _, h, w = current_tensor.shape
        downsampled_tensor = F.interpolate(
            current_tensor, (h // 2 + h % 2, w // 2 + w % 2), mode=mode, align_corners=(H % 2) == 1
        )  # antialias=True
        if size_align:
...
class FrequencySelection(nn.Module):
    def __init__(
        ...
        elif self.lp_type == "laplacian" or self.lp_type == "freq":
            pass
        else:
            raise NotImplementedError
    ...
    def forward(self, x, att_feat=None):
        ...
        if self.lp_type == "avgpool":
            ...
            if self.lowfreq_att:
                freq_weight = self.freq_weight_conv_list[len(x_list)](att_feat)
                # tmp = freq_weight[:, :, len(x_list):len(x_list)+1] * pre_x.reshape(b, self.spatial_group, -1, h, w)
                tmp = freq_weight.reshape(b, self.spatial_group, -1, h, w) * pyramids[-1].reshape(
                    b, self.spatial_group, -1, h, w
                )
                x_list.append(tmp.reshape(b, -1, h, w))
            else:
            ...
            for idx, freq in enumerate(self.k_list):
                ...
                mask[
                    ...
                    round(w / 2 - w / (2 * freq)) : round(w / 2 + w / (2 * freq)),
                ] = 1.0
...
class AdaptiveDilatedConv(ModulatedDeformConv2d):
    ...
    Args:
        ...
        bias (bool or str): If specified as `auto`, it will be decided by the norm_cfg. Bias will be set as True if
            norm_cfg is None, otherwise False.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if conv_type == "conv":
            ...
            self.conv_offset = MultiFreqBandConv(
                self.in_channels, self.deform_groups * 1, freq_band=4, kernel_size=1, dilation=self.dilation
            )
        else:
            raise NotImplementedError
    ...
    def forward(self, x):
        ...
        if hasattr(self, "FS") and (self.pre_fs == False):
        ...
        if hasattr(self, "OMNI_ATT1") and hasattr(self, "OMNI_ATT2"):
            ...
            x = modulated_deform_conv2d(
                ...
                if isinstance(self.PAD, nn.Identity)
                else (0, 0),  # padding
                (1, 1),  # dilation
                self.groups * b,
                self.deform_groups * b,
            )
        elif hasattr(self, "OMNI_ATT"):
            offset = offset.reshape(1, -1, h, w)
            mask = mask.reshape(1, -1, h, w)
    ...
    def ad_forward(self, x):
        ...
        if hasattr(self, "OMNI_ATT1") and hasattr(self, "OMNI_ATT2"):
            ...
            x = modulated_deform_conv2d(
                ...
                self.bias,
                self.stride,
                self.padding if isinstance(self.PAD, nn.Identity) else 0,  # padding
                (1, 1),  # dilation
                self.groups * b,
                self.deform_groups * b,
            )
    ...
    def mix_forward(self, x):
        ...
        if hasattr(self, "OMNI_ATT1") and hasattr(self, "OMNI_ATT2"):
            ...
            x = modulated_deform_conv2d(
                ...
                offset,
                mask,
                adaptive_weight[:, : -self.normal_conv_dim].reshape(
                ...
                self.padding if isinstance(self.PAD, nn.Identity) else 0,  # padding
                (1, 1),  # dilation
                (self.in_channels - self.normal_conv_dim) * b,
                self.deform_groups * b,
            )
            x = x.reshape(b, -1, h, w)
            normal_conv_x = F.conv2d(
                normal_conv_x.reshape(1, -1, h, w),
                adaptive_weight[:, -self.normal_conv_dim :].reshape(
                    -1, self.in_channels // self.groups, self.kernel_size[0], self.kernel_size[1]
                ),
                bias=self.bias,
...
if __name__ == "__main__":
    ...
    # 此模块需要安装mmcv.
    # 如需要更改kernel_size,padding也需要一起更改,padding=kernel_size // 2.
    module = AdaptiveDilatedConv(
