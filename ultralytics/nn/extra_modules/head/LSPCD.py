...
class Detect_LSPCD(Detect):
    ...
    def __init__(self, nc=80, reg_max=16, end2end=False, ch=...):
        ...
        self.stem = nn.Sequential(
            ...
            Conv_GN(self.c_hid, self.c_hid, 1),
            Partial_Conv(self.c_hid, self.c_hid),
            Conv_GN(self.c_hid, self.c_hid, 1),
        )
    ...
    def one2many(self):
        ...
        return {"box_head": self.cv2, "cls_head": self.cv3, "scale_head": self.scale}

    @property
    def one2one(self):
        """Returns the one-to-one head components."""
        return {"box_head": self.one2one_cv2, "cls_head": self.one2one_cv3, "scale_head": self.one2one_scale}
    ...
    def forward_share_head(
        ...
        scale_head: torch.nn.Module = None,
...
class Segment_LSPCD(Detect_LSPCD):
    ...
    def one2many(self):
        ...
        return {"box_head": self.cv2, "cls_head": self.cv3, "scale_head": self.scale, "mask_head": self.cv4}
...
class OBB_LSPCD(Detect_LSPCD):
    ...
    def one2many(self):
        """Returns the one-to-many head components, here for backward compatibility."""
        return {"box_head": self.cv2, "cls_head": self.cv3, "scale_head": self.scale, "angle_head": self.cv4}
    ...
    def forward_share_head(
        ...
        if angle_head is not None:
            bs = x[0].shape[0]  # batch size
            angle = torch.cat([angle_head(x[i]).view(bs, self.ne, -1) for i in range(self.nl)], 2)  # OBB theta logits
...
class Pose_LSPCD(Detect_LSPCD):
    ...
    def one2many(self):
        """Returns the one-to-many head components, here for backward compatibility."""
        return {"box_head": self.cv2, "cls_head": self.cv3, "scale_head": self.scale, "pose_head": self.cv4}
