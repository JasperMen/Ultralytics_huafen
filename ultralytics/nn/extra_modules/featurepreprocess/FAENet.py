"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/TGRS2025-FAENet.png
ultralytics/nn/module_images/TGRS2025-FAENet.md
论文链接：https://ieeexplore.ieee.org/document/10955257.
"""
...
class DilatedConvNet(nn.Module):
    def __init__(self, in_channels, out_channels, dilation, padding, kernel_size):
        ...
        self.dilated_conv = nn.Conv2d(
            in_channels, out_channels, kernel_size=kernel_size, stride=1, padding=padding, dilation=dilation
        )
...
class RFEM(nn.Module):
    def __init__(
        ...
        self.dconv1 = DilatedConvNet(ch_blocks, ch_blocks // 4, kernel_size=3, padding=1, dilation=1)
        self.dconv2 = DilatedConvNet(ch_blocks, ch_blocks // 4, kernel_size=3, padding=2, dilation=2)
        self.dconv3 = DilatedConvNet(ch_blocks, ch_blocks // 4, kernel_size=3, padding=3, dilation=3)
        self.dconv4 = nn.Conv2d(ch_blocks, ch_blocks // 4, kernel_size=7, padding=3)
...
class FAENet(nn.Module):
    def __init__(self, num_high=1, ch_blocks=32, up_ksize=1, high_ch=32, high_ksize=3, ch_mask=32, gauss_kernel=7):
        super().__init__()
        self.num_high = num_high
        ...
        for i in range(self.num_high):
            self.__setattr__(f"up_tag_layer_{i}", Up_tag(up_ksize, ch=3))
            self.__setattr__(f"trans_high_layer_{i}", Trans_high(3, high_ch, 3, high_ksize))
    ...
    def forward(self, x):
        ...
        commom_tag = []
        for i in range(self.num_high):
            tag = self.__getattr__(f"up_tag_layer_{i}")(tag)
            commom_tag.append(tag)

        for i in range(self.num_high):
            trans_pyr = self.__getattr__(f"trans_high_layer_{i}")(pyrs[-2 - i], commom_tag[i])
            trans_pyrs.append(trans_pyr)
...
if __name__ == "__main__":
    ...
    batch_size, in_channel, height, width = 1, 3, 640, 640
    inputs = torch.randn((batch_size, in_channel, height, width)).to(device)

    module = FAENet().to(device)
