...
class FrequencyProjection(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.act = nn.GELU()
        self.res_2 = nn.Sequential(nn.MaxPool2d(3, 1, 1), nn.Conv2d(dim // 4, dim // 4, 1, 1, 0), nn.GELU())
        self.conv_out = nn.Conv2d(dim // 2, dim, 1, 1, 0)

    def forward(self, x):
        """Input: x: (B, C, H, W) Output: x: (B, C, H, W).
        """
        res = x
        x = self.conv_1(x)
...
class Spatial_Attention(nn.Module):
    """Spatial Self-Attention. It supports rectangle window (containing square window).
...
class Spatial_Frequency_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attened_x = (
            attened_x
            * torch.sigmoid(channel_map)
            * torch.sigmoid(reduce(hf, "b c h w -> b c 1 1", "mean").permute(0, 2, 3, 1).contiguous().view(B, 1, C))
        )
        # S-I
