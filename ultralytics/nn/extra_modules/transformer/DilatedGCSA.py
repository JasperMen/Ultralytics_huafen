...
class DilatedGCSA(nn.Module):
    def __init__(self, dim, num_heads=8, window_size=4, overlap_ratio=0.5, dim_head=16, bias=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        ks, vs = (self.unfold(t) for t in (ks, vs))
        ks, vs = (rearrange(t, "b (c j) i -> (b i) j c", c=self.inner_dim) for t in (ks, vs))

        # print(f'qs.shape:{qs.shape}, ks.shape:{ks.shape}, vs.shape:{vs.shape}')
        # split heads
        qs, ks, vs = (rearrange(t, "b n (head c) -> (b head) n c", head=self.num_spatial_heads) for t in (qs, ks, vs))
...
if __name__ == "__main__":
    ...
    # 此模块不支持多尺度训练
    module = DilatedGCSA(channel, num_heads=8).to(device)
