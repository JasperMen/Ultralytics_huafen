"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2026-BinaryAttention.png
ultralytics/nn/module_images/CVPR2026-BinaryAttention.md
...
class STESign(Function):
    """Binarize tensor using sign function. Straight-Through Estimator (STE) is used to approximate the gradient of sign
    function.
    """
    ...
    def forward(ctx: Any, x: torch.Tensor) -> BinaryTensor:
        ...
        Returns:
            Sign(x) = (x>=0) - (x<0): Output type is float tensor where each element is either -1 or 1.
        """
    ...
    def __init__(
        ...
        if self.attn_bias:  # dense bias
            ...
            relative_position_index = torch.zeros(
                size=(input_size[0] * input_size[1] + 1,) * 2, dtype=relative_coords.dtype
            )
        ...
        if self.attn_quant:
            ...
            if self.attn_bias:
                relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
                    self.input_size[0] * self.input_size[1] + 1, self.input_size[0] * self.input_size[1] + 1, -1
                )
...
if __name__ == "__main__":
    ...
    module = BinaryAttention(channel, num_heads=8).to(device)
