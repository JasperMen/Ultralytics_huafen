"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2023-Cascaded Group Attention.png
...
class CascadedGroupAtt(torch.nn.Module):
    ...
    def __init__(self, dim, key_dim, num_heads=4, attn_ratio=4, resolution=14, kernels=None):
        if kernels is None:
            kernels = [5, 5, 5, 5]
        super().__init__()
...
class CascadedGroupAttention(torch.nn.Module):
    ...
    def __init__(self, dim, key_dim=16, num_heads=4, attn_ratio=4, resolution=14, window_resolution=7, kernels=None):
        if kernels is None:
            kernels = [5, 5, 5, 5]
        super().__init__()
        self.dim = dim
        ...
        self.attn = CascadedGroupAtt(
            dim, key_dim, num_heads, attn_ratio=attn_ratio, resolution=window_resolution, kernels=kernels
        )
