"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/ICLR2025-PolaLinearAttention.png
    ...
    def forward(self, x):
        ...
        if x.ndim == 4:
            ...
            if (H, W) != (self.h, self.w):
                raise ValueError(f"PolaLinearAttention expected spatial size {(self.h, self.w)}, got {(H, W)}.")
            x = x.flatten(2).transpose(1, 2).contiguous()
        elif x.ndim != 3:
            raise ValueError(f"PolaLinearAttention expects input as [B, N, C] or [B, C, H, W], got {tuple(x.shape)}")

        B, N, C = x.shape
        if N != self.h * self.w:
            raise ValueError(f"PolaLinearAttention expected N={self.h * self.w}, got {N}.")
        ...
        if restore_bchw:
            return x.transpose(1, 2).reshape(B, C, self.h, self.w).contiguous()
        return x
...
if __name__ == "__main__":
    ...
    # 此模块不支持多尺度训练，hw的参数是一个元组/列表，其为当前特征图的height,width。
