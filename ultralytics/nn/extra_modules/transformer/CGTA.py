...
class CGTA(nn.Module):
    ...
    This file is a standalone extraction of the original project class with all project-local dependencies inlined so it
    ...
    def forward(self, x):
        ...
        cpe = self.cpe(
            v.transpose(1, 2).reshape(B, -1, C).transpose(1, 2).contiguous().view(B, C, H // scale, W // scale)
        )
