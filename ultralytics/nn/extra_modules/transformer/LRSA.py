...
class LRSA(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.q_pooled_size > 1:
            ...
            q_pooled_size = (
                (self.q_pooled_size, round(W * float(self.q_pooled_size) / H + self.eps))
                if W >= H
                else (round(H * float(self.q_pooled_size) / W + self.eps), self.q_pooled_size)
            )
        ...
        for pooled_size, l in zip(self.pooled_sizes, self.d_convs):
            pooled_size = (
                (pooled_size, round(W * pooled_size / H + self.eps))
                if W >= H
                else (round(H * pooled_size / W + self.eps), pooled_size)
            )
