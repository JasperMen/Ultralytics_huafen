"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2023-BiLevelRoutingAttention.png
...
def _seq2grid(x: Tensor, region_h: int, region_w: int, region_size: tuple[int]):
    ...
    Returns:
        x: (bs, C, H, W).
    """
    bs, nhead, _nregion, _reg_size_square, head_dim = x.size()
...
def regional_routing_attention_torch(
    ...
    kv_region_size: tuple[int] | None = None,
    auto_pad=True,
) -> Tensor:
    """
    Args:
    ...
    # TODO: is separate gathering slower than fused one (our old version) ?
...
class BiLevelRoutingAttention_nchw(nn.Module):
    ...
    * uses nchw input format to avoid frequent permutation
    ...
    When the size of inputs is not divisible by the region size, there is also a numerical difference than legacy
    ...
    * different pooling behavior (count_include_pad=False)
    ...
    Current implementation is more reasonable, hence we do not keep backward numerical compatibility
    """

    def __init__(
        self, dim, num_heads=8, n_win=7, qk_scale=None, topk=4, side_dwconv=3, auto_pad=False, attn_backend="torch"
    ):
        super().__init__()
        # local attention setting
        self.dim = dim
    ...
    def forward(self, x: Tensor, ret_attn_mask=False):
        """
        Args:
            x: NCHW tensor, better to be channel_last
