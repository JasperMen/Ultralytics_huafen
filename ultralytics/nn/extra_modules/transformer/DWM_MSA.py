...
class DWM_MSA(nn.Module):
    ...
    def forward(self, x):
        ...
        q1, k1, v1 = (
            rearrange(t, "b (h b0) (w b1) c -> b (h w) (b0 b1) c", b0=w_size1[0], b1=w_size1[1]) for t in (q1, k1, v1)
        )
        q1, k1, v1 = (rearrange(t, "b n mm (h d) -> b n h mm d", h=self.heads) for t in (q1, k1, v1))
        q1 *= self.scale
        ...
        q2, k2, v2 = (
            rearrange(t, "b (h b0) (w b1) c -> b (h w) (b0 b1) c", b0=w_size1[0], b1=w_size1[1]) for t in (q2, k2, v2)
        )
        q2, k2, v2 = (t.permute(0, 2, 1, 3) for t in (q2.clone(), k2.clone(), v2.clone()))
        q2, k2, v2 = (rearrange(t, "b n mm (h d) -> b n h mm d", h=self.heads) for t in (q2, k2, v2))
        q2 *= self.scale
        ...
        out_1 = rearrange(
            out_1, "b (h w) (b0 b1) c -> b (h b0) (w b1) c", h=h // w_size1[0], w=w // w_size1[1], b0=w_size1[0]
        )

        # local branch of window size 2
        q3, k3, v3 = (
            rearrange(t, "b (h b0) (w b1) c -> b (h w) (b0 b1) c", b0=w_size2[0], b1=w_size2[1]) for t in (q3, k3, v3)
        )
        q3, k3, v3 = (rearrange(t, "b n mm (h d) -> b n h mm d", h=self.heads) for t in (q3, k3, v3))
        q3 *= self.scale
        ...
        q4, k4, v4 = (
            rearrange(t, "b (h b0) (w b1) c -> b (h w) (b0 b1) c", b0=w_size2[0], b1=w_size2[1]) for t in (q4, k4, v4)
        )
        q4, k4, v4 = (t.permute(0, 2, 1, 3) for t in (q4.clone(), k4.clone(), v4.clone()))
        q4, k4, v4 = (rearrange(t, "b n mm (h d) -> b n h mm d", h=self.heads) for t in (q4, k4, v4))
        q4 *= self.scale
        ...
        out_2 = rearrange(
            out_2, "b (h w) (b0 b1) c -> b (h b0) (w b1) c", h=h // w_size2[0], w=w // w_size2[1], b0=w_size2[0]
        )
