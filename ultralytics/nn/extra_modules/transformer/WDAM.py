"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2026-WDAM.png
ultralytics/nn/module_images/CVPR2026-WDAM.md
...
class WDAM(nn.Module):
    def __init__(self, dim, num_heads=8, window_size=5, shift_size=2, bias=False):
        ...
        self.high_out = nn.Sequential(
            nn.Conv2d(dim * 3, dim * 3, kernel_size=3, padding=1, groups=3, bias=bias), nn.ReLU(inplace=True)
        )
    ...
    def window_attention(self, q, k, v):
        """q,k,v: (B_win, num_heads, head_dim, N) 返回: (B_win, num_heads, head_dim, N).
        """
...
if __name__ == "__main__":
    ...
    module = WDAM(channel, num_heads=8).to(device)
