...
__all__ = ["A3Conv", "A3Fusion2", "A3Fusion3"]
...
def _safe_group_count(channels: int, preferred: int | None) -> int:
...
def build_norm(norm_cfg: bool | str | nn.Module | dict | None, channels: int) -> nn.Module:
...
class A3Conv(nn.Module):
    ...
    def __init__(
        ...
        bias: bool = False,
        norm_cfg: bool | str | nn.Module | dict | None = "BN",
    ) -> None:
        super().__init__()
...
class RepVggBlock(nn.Module):
    def __init__(
        ...
        self.act = (
            nn.Identity() if act is False or act is None else get_activation(act) if isinstance(act, str) else act
        )
...
class A3Resampler(nn.Module):
    def __init__(
        ...
        channels: int,
        act: str | nn.Module | bool = "gelu",
        dcn_norm: bool | str | nn.Module | dict | None = "LN",
        dcn_group: int = 4,
...
class Fusion(nn.Module):
    ...
    def forward(self, x: Sequence[torch.Tensor]) -> torch.Tensor:
        ...
        levels_weight = self.weight_levels(
            torch.cat([layer(feat) for layer, feat in zip(self.weight_layers, fusion_feats)], dim=1)
        )
...
class Reassemble(nn.Module):
    def __init__(
        ...
        act: str | nn.Module | bool = "gelu",
        norm: bool | str | nn.Module | dict | None = "LN",
    ) -> None:
        super().__init__()
