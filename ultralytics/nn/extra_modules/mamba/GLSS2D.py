...
class LocalityFeedForward(nn.Module):
    def __init__(
        self, in_dim, out_dim, stride, expand_ratio=4.0, act="hs+se", reduction=4, wo_dp_conv=False, dp_first=False
    ):
        """
...
class shiftmlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.0, shift_size=5):
        ...
        self.conv = LocalityFeedForward(
            in_features, out_features, 1, expand_ratio=4.0, act="hs+se", reduction=4, wo_dp_conv=False, dp_first=False
        )
...
class shiftedBlock(nn.Module):
    def __init__(
        self, dim, mlp_ratio=4.0, drop=0.0, shift_size=4, drop_path=0.0, act_layer=nn.GELU, norm_layer=nn.BatchNorm2d
    ):
        super().__init__()
        ...
        self.mlp = shiftmlp(
            in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, shift_size=shift_size
        )
        self.apply(self._init_weights)
...
class GLSS2D(nn.Module):
    def __init__(self, dim, d_state=16, dropout=0.0):
        ...
        self.mff = shiftedBlock(dim=dim, mlp_ratio=1, drop=0.0, drop_path=0, norm_layer=nn.BatchNorm2d, shift_size=4)
        self.mlp = shiftmlp(in_features=dim // 2, hidden_features=2 * dim, act_layer=nn.GELU, drop=0.0, shift_size=4)
