"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/CVPR2025-VMM.md
ultralytics/nn/module_images/CVPR2025-VMM.png
    ...
    if shift_len == 0:
        ...
        for i in range(0, ((W - shift_len) // scan_len) + 1, 2):
            inp[:, :, :, (shift_len + i * scan_len) : (shift_len + (i + 1) * scan_len)] = inp[
                :, :, :, (shift_len + i * scan_len) : (shift_len + (i + 1) * scan_len)
            ].flip(dims=[-2])
    ...
    if shift_len == 0:
        for hi in range(H // 2):
            for wi in range(W // scan_len):
                inp[:, :, 2 * hi + 1, wi * scan_len : (wi + 1) * scan_len] = inp[
                    :, :, 2 * hi + 1, wi * scan_len : (wi + 1) * scan_len
                ].flip(dims=[-1])
    else:
        for hi in range(H // 2):
...
def inverse_ids_generate(origin_ids, K=4):
    """Input: origin_ids: (B, K, C, L) Output: (B, K, C, L) Note: C is set to 1 for speeding up.
    """
...
def mair_ids_generate(inp_shape, scan_len=4, K=4):
    _inp_b, _inp_c, inp_h, inp_w = inp_shape
...
def mair_shift_ids_generate(inp_shape, scan_len=4, shift_len=0, K=4):
    inp_b, _inp_c, inp_h, inp_w = inp_shape
...
def mair_ids_inverse(inp, xs_scan_ids, shape=None):
    """inp: (B, K, -1, L) xs_scan_ids: (1, K, 1, L).
    """
    B, K, _, L = inp.shape
    xs_scan_ids = xs_scan_ids.reshape(K, L)
    if not shape:
...
class VMM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.self_attention = VMM_Layer(
            d_model=hidden_dim, d_state=d_state, expand=ssm_ratio, dropout=attn_drop_rate, **kwargs
        )
    ...
    def _generate_ids(self):
        ...
        xs_shift_scan_ids, xs_shift_inverse_ids = mair_shift_ids_generate(
            inp_shape=(1, 1, H, W), scan_len=self.scan_len, shift_len=self.scan_len // 2
        )  # [B,H,W,C]
    ...
    def forward(self, input):
        ...
        self.xs_shift_scan_ids = self.xs_shift_scan_ids.to(x.device)
        self.xs_shift_inverse_ids = self.xs_shift_inverse_ids.to(x.device)
        self.xs_scan_ids = self.xs_scan_ids.to(x.device)
        self.xs_inverse_ids = self.xs_inverse_ids.to(x.device)

        if self.shift_size > 0:
            x = self.self_attention(x, (self.xs_shift_scan_ids, self.xs_shift_inverse_ids))
        else:
            x = self.self_attention(x, (self.xs_scan_ids, self.xs_inverse_ids))
...
if __name__ == "__main__":
    ...
    # 此模块不支持多尺度训练，inp_shape的参数是一个元组，其为当前特征图的height,width。
    module = VMM(channel, inp_shape=(height, width)).to(device)
