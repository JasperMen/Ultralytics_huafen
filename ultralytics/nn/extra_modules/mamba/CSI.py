...
class CSI(nn.Module):   
    ...
    def forward(self, x):
        ...
        x_norm = F.layer_norm(x_flat.float(), self.norm.normalized_shape, self.norm.weight.float(), self.norm.bias.float(), self.norm.eps).to(dtype=x_dtype)    
    
        x1, x2, x3, x4 = torch.chunk(x_norm, 4, dim=2)
        x_mamba1 = self.mlp(F.layer_norm(self.mamba(x1).float(), self.norm1.normalized_shape, self.norm1.weight.float(), self.norm1.bias.float(), self.norm1.eps).to(dtype=x_dtype)) + self.skip_scale * x1
        x_mamba2 = self.mlp(F.layer_norm(self.mamba(x2).float(), self.norm1.normalized_shape, self.norm1.weight.float(), self.norm1.bias.float(), self.norm1.eps).to(dtype=x_dtype)) + self.skip_scale * x2     
        x_mamba3 = self.mlp(F.layer_norm(self.mamba(x3).float(), self.norm1.normalized_shape, self.norm1.weight.float(), self.norm1.bias.float(), self.norm1.eps).to(dtype=x_dtype)) + self.skip_scale * x3   
        x_mamba4 = self.mlp(F.layer_norm(self.mamba(x4).float(), self.norm1.normalized_shape, self.norm1.weight.float(), self.norm1.bias.float(), self.norm1.eps).to(dtype=x_dtype)) + self.skip_scale * x4
