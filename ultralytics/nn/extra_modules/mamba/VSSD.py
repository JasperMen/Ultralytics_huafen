"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/ICCV2025-VSSD.md
ultralytics/nn/module_images/ICCV2025-VSSD.png
...
class Mamba2(nn.Module):
    ...
    def non_casual_linear_attn(self, x, dt, A, B, C, D, H=None, W=None):
        """non-casual attention duality of mamba v2 x: (B, L, H, D), equivalent to V in attention dt: (B, L, nheads) A:
        (nheads) or (d_inner, d_state) B: (B, L, d_state), equivalent to K in attention C: (B, L, d_state),
        equivalent to Q in attention D: (nheads), equivalent to the skip connection.
        """
        ...
        if getattr(self, "__DEBUG__", False):
            ...
            self.__data__ = {"dA": A_mat, "H": H, "W": W, "V": V}
    ...
    def forward(self, u, H, W, seq_idx=None):
        """u: (B,C,H,W) Returns: same shape as u.
        """
        batch, _seqlen, _dim = u.shape
        ...
        if self.linear_attn_duality:
            y = self.non_casual_linear_attn(
                rearrange(x, "b l (h p) -> b l h p", p=self.headdim), dt, A, B, C, self.D, H, W
            )
        else:
...
if __name__ == "__main__":
    ...
    module = VMAMBA2Block(channel).to(device)
