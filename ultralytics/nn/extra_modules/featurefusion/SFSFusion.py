...
class SFSFusion(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, n_heads=8, n_points=4):
        ...
        self.query_Conv = nn.Sequential(
            nn.Conv2d(in_dim[0], dim, kernel_size=3, padding=1), nn.BatchNorm2d(dim), nn.ReLU(inplace=True)
        )
        self.key_Conv = nn.Sequential(
            nn.Conv2d(in_dim[1], dim, kernel_size=3, padding=1), nn.BatchNorm2d(dim), nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x) -> Tensor:
        ...
        shared_offsets = self.offset_base.view(self.n_heads, 1, self.n_points, 2) + offsets_residual.view(
