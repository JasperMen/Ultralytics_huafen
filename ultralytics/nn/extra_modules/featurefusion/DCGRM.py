"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/自研模块-DCGRM.png
ultralytics/nn/module_images/自研模块-DCGRM.md.
"""
...
class DCGRM(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv1x1 = nn.ModuleList(
            [Conv(c, out_channel, 1) if c != out_channel else nn.Identity() for c in in_channel]
        )

        # Discrepancy branch: 3×3 conv to encode cross-branch mismatch (edge-like features)
        self.disc_branch = Conv(out_channel, out_channel, 3, act=nn.ReLU)
        ...
        self.conv_query = Conv(out_channel, out_channel, 3, act=nn.ReLU)
        ...
        self.conv_scale = Conv(out_channel, out_channel, 3, act=nn.ReLU)
...
if __name__ == "__main__":
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for half precision validation.")
    half_inputs_1 = torch.randn((batch_size, channel_1, height, width), device="cuda").half()
    half_inputs_2 = torch.randn((batch_size, channel_2, height, width), device="cuda").half()
    half_module = DCGRM([channel_1, channel_2], ouc_channel).cuda().half().eval()
    with torch.no_grad():
        half_outputs = half_module([half_inputs_1, half_inputs_2])
    print(
        GREEN
        + f"half inputs1.size:{half_inputs_1.size()} half inputs2.size:{half_inputs_2.size()} half outputs.size:{half_outputs.size()}"
        + RESET
    )
