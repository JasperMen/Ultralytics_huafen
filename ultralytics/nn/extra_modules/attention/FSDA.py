"""
本文件由BiliBili：魔傀面具整理
ultralytics/nn/module_images/FSDA.png
ultralytics/nn/module_images/FSDA.md.
"""
...
class FSDA(nn.Module):
    ...
    Decomposes input features into low-frequency (global structure) and high-frequency (edge/detail) components via FFT
    with a learnable Gaussian soft mask, then applies Channel Attention to low-frequency and Spatial Attention to
    high-frequency, fusing results through an adaptive gate.
    """
    ...
    def frequency_decompose(self, x):
        ...
        x_low = torch.fft.ifft2(torch.fft.ifftshift(f_low, dim=(-2, -1)), dim=(-2, -1), norm="ortho").real
        x_high = torch.fft.ifft2(torch.fft.ifftshift(f_high, dim=(-2, -1)), dim=(-2, -1), norm="ortho").real
    ...
    def adaptive_fusion(self, x_cam, x_sam):
        ...
        # Compute fusion gate
        gate = self.sigmoid(self.gate_fc_cam(gap_cam) + self.gate_fc_sam(gap_sam))  # [B, C]
    ...
    def forward(self, x):
        ...
        x_low = x_low.to(dtype=x_refined.dtype)
        x_high = x_high.to(dtype=x_refined.dtype)
