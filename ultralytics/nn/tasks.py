...
class BaseModel(torch.nn.Module):
    ...
    def _profile_one_layer(self, m, x, dt):
        ...
        if m == self.model[0]:
            ...
            # 打印表头，带有分隔线和颜色
            LOGGER.info(f"\n{BOLD}{CYAN}{'=' * 100}{RESET}")
            LOGGER.info(f"{BOLD}{ORANGE}{'Layer Profiling Results':^100}{RESET}")
            LOGGER.info(f"{BOLD}{CYAN}{'=' * 100}{RESET}")
            LOGGER.info(f"{BOLD}{YELLOW}{'time (ms)':>12s} {'GFLOPs':>12s} {'params':>12s}  {'module':<50}{RESET}")
    ...
    def __init__(self, cfg="yolo26n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            m.stride = torch.tensor(
                [s / x.shape[-2] for x in _forward(torch.zeros(2, ch, s, s).to(self.device))]
            )  # forward
...
def parse_model(d, ch, verbose=True):
    """Parse a YOLO model.yaml dictionary into a PyTorch model. 将 YOLO 的 model.yaml 字典解析为 PyTorch 模型。.
    ...
    block_repeat_modules = frozenset({C3_Block, C2f_Block, C3k2_Block, MANet})

    # 万物皆可融Block集合，这些模块不受重复次数的影响
    block_modules = frozenset({MetaFormer_Block, MetaFormer_Mona, MetaFormer_SEFN, MetaFormer_Mona_SEFN})
    ...
    # upsample集合
    upsample_modules = frozenset({CARAFE, Converse2D_Up, DSUB, DySample, DySampleFusion, EUCB, EUCB_SC, WaveletUnPool})
    ...
    module_modules = frozenset(
        {
            ...
            MAC,
            MambaOut,
            MambaOut_DilatedReparamConv,
            MambaOut_UniRepLKBlock,
            MFEblock,
            MSBlock,
            MSCB,
            PartialNetBlock,
    ...
    # head集合
    head_modules = frozenset({Detect_LQE, Segment_LQE, Segment26_LQE, OBB_LQE, OBB26_LQE, Pose_LQE, Pose26_LQE})
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # 检测头和分割头处理 (Detect, Segment, Pose, OBB 等)
            ...
            if type(args) is list:
                ...
                def _resolve_module_from_name(module_name):
                    if isinstance(module_name, str):
                        try:
                            return (
                                getattr(torch.nn, module_name[3:]) if "nn." in module_name else globals()[module_name]
                            )
                ...
                def _resolve_module_param_value(value):
                    if isinstance(value, dict):
                        if "module" in value:
                            ...
                            return (
                                partial(nested_module, **nested_param)
                                if isinstance(nested_param, dict)
                                else partial(nested_module, nested_param)
                            )
                ...
                for j, a in enumerate(args):
                    if isinstance(a, str):
                        ...
                        if "module" in a:
                            ...
                            args[j] = (
                                partial(module_, **module_param)
                                if isinstance(module_param, dict)
                                else partial(module_, module_param)
                            )
                        if "selfatt" in a:
                            selfatt = a["selfatt"]
            ...
            {Detect_LSPCD, Segment_LSPCD, Segment26_LSPCD, OBB_LSPCD, OBB26_LSPCD, Pose_LSPCD, Pose26_LSPCD}
        ):
            # 添加参数: reg_max, end2end, 以及来自 backbone 不同层的通道列表
            args.extend([reg_max, end2end, [ch[x] for x in f]])
