...
class AAttn(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        )  # [B, self.num_heads, self.head_dim * 3, N]
...
class ABlock(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, mlp_ratio: float = 1.2, area: int = 1, version: str = "v1"):
        ...
        assert version in ["v1", "turbo"], f"ABlock must be v1 or turbo, but got {version}"
        AAttn_block = AAttn if version == "v1" else AAttn_Turbo

        self.attn = AAttn_block(dim, num_heads=num_heads, area=area)
...
class A2C2f(nn.Module):
    ...
    def __init__(
        ...
        self.m = nn.ModuleList(
            nn.Sequential(*(ABlock(c_, c_ // 32, mlp_ratio, area, version) for _ in range(2)))
            if a2
